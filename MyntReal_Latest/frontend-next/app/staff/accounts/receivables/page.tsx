...
export default function ReceivablesCustomersPage() {
  ...
  useEffect(() => {
    ...
    const fetchCustomers = async () => {
      try {
        setLoading(true);
        // Using generic customers/receivables endpoint
        ...
        if (res.ok) {
          ...
          setCustomers([]); // Fallback to empty
        }
      } catch (err: any) {
        console.warn("Failed to fetch customers, using empty state", err);
        setCustomers([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-2">Manage B2B customers, accounts receivable, and customer ledgers.</p>
        </div>
        <div className="flex space-x-3">
          <Link href="/staff/accounts/invoices" className="px-4 py-2 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg shadow-sm hover:bg-gray-50 transition-colors">
            <i className="fas fa-file-invoice mr-2"></i> Sales Invoices
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-green-500">
          <p className="text-xs font-bold text-gray-500 uppercase">Total Receivables</p>
          <p className="text-2xl font-bold text-green-600 mt-1">
            ₹{customers.reduce((sum, c) => sum + (c.outstanding_balance > 0 ? c.outstanding_balance : 0), 0).toFixed(2)}
          </p>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-red-500">
          ...
          <p className="text-2xl font-bold text-red-600 mt-1">
            ₹{customers.reduce((sum, c) => sum + (c.outstanding_balance < 0 ? Math.abs(c.outstanding_balance) : 0), 0).toFixed(2)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-16 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="text-gray-500 mb-4">There are currently no B2B customers matching your criteria.</p>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Customer Code</th>
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Customer Name</th>
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Region/Category</th>
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Contact</th>
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">YTD Sales (₹)</th>
                  <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Receivable Balance (₹)</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredCustomers.map(customer => (
                  <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-4 text-sm font-medium text-indigo-600">{customer.customer_code}</td>
                    <td className="p-4 text-sm font-bold text-gray-900">{customer.customer_name}</td>
                    <td className="p-4 text-sm text-gray-600">{customer.category}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm font-medium text-gray-900 text-right">{customer.total_sales.toFixed(2)}</td>
                    <td className="p-4 text-right">
                      {customer.outstanding_balance > 0 ? (
                        <span className="text-sm font-bold text-green-600">{customer.outstanding_balance.toFixed(2)} Dr</span>
                      ) : customer.outstanding_balance < 0 ? (
                        <span className="text-sm font-bold text-red-600">{Math.abs(customer.outstanding_balance).toFixed(2)} Cr</span>
                      ) : (
                    ...
                    <td className="p-4 text-center">
                      <Link href={`/staff/accounts/party-ledger?party_id=${customer.id}`} className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                        Ledger
                      </Link>
                    </td>
                  </tr>
