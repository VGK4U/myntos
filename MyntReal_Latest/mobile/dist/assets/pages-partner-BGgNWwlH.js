      ...
      <div class="partner-leads-page">
        ...
        <div class="leads-content">
          <div class="status-tabs">
            ...
            ${q.map(a=>`<button class="status-tab" data-status="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</button>`).join("")}
          </div>
...
Destination: ${r.destination_phone||"—"}`):alert(`Call initiation failed: ${r?.detail||r?.message||"Unknown error"}`)}catch(r){alert(`Call failed: ${r.message||"Error"}`)}})}),document.querySelectorAll(".open-partner-wa-btn").forEach(a=>{a.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const o=a.dataset.phone||"",r=a.dataset.name||"Customer",n=a.dataset.id||"",s=a.dataset.cat||"";o&&P.open({phone:o,name:r,leadId:n,context:s})})})}showAddLeadModal(){const e=document.createElement("div");e.id="addLeadModal",e.innerHTML=`
    ...
    </div>`}}function A(c){const e={SUBMITTED:["#451a03","#f59e0b"],ACKNOWLEDGED:["#1e3a5f","#60a5fa"],FULFILLED:["#064e3b","#10b981"],CANCELLED:["#1f2937","#9ca3af"]},[t,a]=e[c]||["#1f2937","#9ca3af"];return`<span style="background:${t};color:${a};padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">${c}</span>`}class V{container;cart=new Map;companies=[];activeTab="catalog";searchTimer=null;allItems=[];requests=[];constructor(e){this.container=e}async init(){this.render(),await this.loadCompanies(),await this.loadCatalog()}render(){this.container.innerHTML=`
