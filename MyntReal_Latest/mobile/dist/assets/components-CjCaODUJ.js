...
import{v as k,j as O,d as v,b as x,r as B,t as A,M as j,a as W}from"./services-CYnl_Ppy.js";import{L as M}from"./vendor-B1GRRwIS.js";import{_ as G}from"./vendor-capacitor-plugins-DTPsYLMi.js";import{gpsService as Y}from"./services-gps-B7OAc6kD.js";class Q{modalEl=null;uiState="CLOSED";currentOptions=null;enteredNumber="";rawNumber="";isPresetNumber=!1;isDtmfOpen=!1;unsubscribeTelephony=null;currentSession=null;floatingPos={x:0,y:0};pillPos={x:0,y:0};isDraggingWindow=!1;isDraggingPill=!1;dragStartPointer={x:0,y:0};dragStartPos={x:0,y:0};hasInitializedPosition=!1;lastNotifiedLeadId=null;constructor(){typeof window<"u"&&(window.addEventListener("keydown",e=>{e.key==="Escape"&&this.isOpen()&&(this.uiState==="ACTIVE_FLOATING"?this.minimize():this.uiState==="DIALER"&&this.close())}),window.addEventListener("resize",()=>{this.isOpen()&&(this.clampPositions(),this.applyPositions())}))}isOpen(){return this.modalEl!==null&&document.body.contains(this.modalEl)&&this.uiState!=="CLOSED"}getUIState(){return this.uiState}open(e){this.currentOptions=e;const t=(e.phoneNumber||"").replace(/[^\d+]/g,"");this.enteredNumber=t,this.rawNumber=t,this.isPresetNumber=!!t,this.isDtmfOpen=!1,this.hasInitializedPosition=!1,this.unsubscribeTelephony&&this.unsubscribeTelephony(),this.unsubscribeTelephony=k.subscribe(i=>{this.currentSession=i,this.handleTelephonyStateUpdate(i)});const a=k.isCallActive();this.uiState=a?"ACTIVE_FLOATING":"DIALER",this.render(),this.loadLeadContext(e.entityId),e.autoStart&&this.enteredNumber&&this.startCall()}minimize(){this.uiState!=="CLOSED"&&(this.uiState="MINIMIZED",this.updateVisibility())}restore(){this.uiState!=="CLOSED"&&(this.uiState="ACTIVE_FLOATING",this.updateVisibility(),this.updateSessionUI())}close(){if(this.currentSession&&(k.isCallActive()||["connecting","ringing","connected","held"].includes(this.currentSession.state))){this.minimize();return}this.teardown()}forceCloseAndHangup(){this.currentSession&&k.isCallActive()&&k.endCall(),this.teardown()}teardown(){this.unsubscribeTelephony&&(this.unsubscribeTelephony(),this.unsubscribeTelephony=null),this.modalEl&&(this.modalEl.remove(),this.modalEl=null),this.uiState="CLOSED",this.currentOptions=null,this.currentSession=null,this.hasInitializedPosition=!1,this.lastNotifiedLeadId=null}handleTelephonyStateUpdate(e){if(!this.modalEl||!document.body.contains(this.modalEl))if(e.state!=="idle")this.render();else return;if(k.isCallActive()){this.uiState==="DIALER"&&(this.uiState="ACTIVE_FLOATING");const a=Number(this.currentOptions?.entityId);a&&(this.currentOptions?.entityType==="lead"||!this.currentOptions?.entityType)&&this.lastNotifiedLeadId!==a&&(this.lastNotifiedLeadId=a,O.notifyCallActive(a))}else if(this.lastNotifiedLeadId=null,e.state==="ended"){this.uiState="ENDED_SUMMARY";const a=this.currentOptions,i=e.durationSeconds||0;O.clearCallActive(),a?.entityId&&window.dispatchEvent(new CustomEvent("myntos:lead-call-ended",{detail:{leadId:a.entityId,name:a.name,phoneNumber:a.phoneNumber,durationSeconds:i,categoryName:a.categoryName,status:a.status}})),setTimeout(()=>{this.uiState==="ENDED_SUMMARY"&&this.teardown()},1600)}else if(e.state==="idle"&&this.uiState!=="DIALER"){O.clearCallActive(),this.teardown();return}this.updateVisibility(),this.updateSessionUI()}maskPhone(e){if(!e)return"—";const t=e.replace(/\D/g,"");if(t.length<6)return e;const a=t.slice(-10);return v.isMR10001()?a.length===10?`+91 ${a.slice(0,5)} ${a.slice(5)}`:e:`+91 ${a.slice(0,2)}••••${a.slice(-4)}`}getDialInputDisplay(){return!v.isMR10001()&&this.isPresetNumber&&this.enteredNumber?this.maskPhone(this.enteredNumber):this.enteredNumber}formatDuration(e){const t=Math.floor(e/60).toString().padStart(2,"0"),a=(e%60).toString().padStart(2,"0");return`${t}:${a}`}initPositions(){if(this.hasInitializedPosition||typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),i=440,n=Math.max(12,Math.round((e-a)/2)),s=Math.max(20,Math.round((t-i)/2));this.floatingPos={x:n,y:s},this.pillPos={x:Math.max(12,e-260),y:Math.max(20,t-100)},this.hasInitializedPosition=!0}clampPositions(){if(typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),i=440,n=12,s=Math.max(12,e-a-12),l=12,o=Math.max(12,t-i-12);this.floatingPos.x=Math.min(Math.max(this.floatingPos.x,n),s),this.floatingPos.y=Math.min(Math.max(this.floatingPos.y,l),o);const d=240,r=54,u=12,p=Math.max(12,e-d-12),m=12,g=Math.max(12,t-r-12);this.pillPos.x=Math.min(Math.max(this.pillPos.x,u),p),this.pillPos.y=Math.min(Math.max(this.pillPos.y,m),g)}applyPositions(){const e=this.modalEl?.querySelector("#spModalDialog");e&&(this.uiState==="ACTIVE_FLOATING"||this.uiState==="DIALER")&&(e.style.transform=`translate3d(${this.floatingPos.x}px, ${this.floatingPos.y}px, 0)`);const t=this.modalEl?.querySelector("#spMinimizedPill");t&&this.uiState==="MINIMIZED"&&(t.style.transform=`translate3d(${this.pillPos.x}px, ${this.pillPos.y}px, 0)`)}render(){this.modalEl&&this.modalEl.remove(),this.initPositions(),this.clampPositions();const{name:e,entityType:t}=this.currentOptions||{},a=e||"Customer Lead",i=t?t.toUpperCase():"LEAD";this.modalEl=document.createElement("div"),this.modalEl.id="myntosCentralSoftphoneModal",this.modalEl.style.cssText=`
          ...
          <div id="spInCallView" style="display: ${this.uiState==="ACTIVE_FLOATING"||this.uiState==="ENDED_SUMMARY"?"flex":"none"} !important; position: absolute !important; inset: 0 !important; background: linear-gradient(180deg, #0f172a 0%, #1e293b 100%) !important; color: #ffffff !important; padding: 18px 16px !important; z-index: 20 !important; flex-direction: column !important; justify-content: space-between !important; align-items: center !important;">
            ...
            <div style="display: flex !important; flex-direction: column !important; align-items: center !important; gap: 12px !important; width: 100% !important;">
              ...
              <div id="spInCallVolumeWrap" style="display: flex !important; align-items: center !important; justify-content: center !important; gap: 8px !important; width: 100% !important; max-width: 220px !important; padding: 4px 10px !important; background: rgba(255,255,255,0.06) !important; border-radius: 12px !important; border: 1px solid rgba(255,255,255,0.1) !important; box-sizing: border-box !important;">
                ...
                <input id="spInCallVolumeSlider" type="range" min="0" max="100" value="${Math.round(k.getVolume()*100)}" style="width: 100% !important; height: 4px !important; accent-color: #38bdf8 !important; cursor: pointer !important;" title="Adjust call volume" />
                <span style="color: #94a3b8 !important; font-size: 11px !important;">🔊</span>
              </div>
    ...
    `}static getPortalDashboard(){const e=B.getPortal();return e==="mnr"?"mnr-dashboard":e==="partner"?"partner-dashboard":e==="vgk"?"vgk-member-hub":"progress"}static attachListeners(e){let{showMenu:t,showBack:a=!1,rightAction:i,showLogout:n=!1}=e;t===void 0&&(t=!!document.getElementById("hamburgerBtn")),B.getPortal()==="vgk"&&(n=!0),t&&document.getElementById("hamburgerBtn")?.addEventListener("click",()=>{ae().open()}),a&&document.getElementById("backBtn")?.addEventListener("click",()=>{A.goBack()||A.navigate(U.getPortalDashboard())}),i&&document.getElementById("headerActionBtn")?.addEventListener("click",i.onClick),n&&document.getElementById("logoutBtn")?.addEventListener("click",async()=>{confirm("Are you sure you want to logout?")&&await v.logout()})}static attachBackHandler(){document.getElementById("backBtn")?.addEventListener("click",()=>{A.goBack()||A.navigate(U.getPortalDashboard())})}}class xe{constructor(e){this.options=e,this.trackPoints=e.trackPoints,this.stops=e.stops||[],this.onViewChange=e.onViewChange}options;map=null;container=null;trackPoints=[];stops=[];routeLine=null;progressLine=null;currentMarker=null;startMarker=null;endMarker=null;stopMarkers=[];currentView="street";tileLayers={};playbackIndex=0;isPlaying=!1;playbackInterval=null;playbackSpeed=2;onViewChange;addressCache=new Map;mount(){if(this.container=document.getElementById(this.options.containerId),!this.container){console.error("[LeafletMap] Container not found:",this.options.containerId);return}this.render(),this.initMap()}render(){this.container&&(this.container.innerHTML=`
...
`);e.textContent=m}async executeShare(e){const t=document.getElementById("usmStaffSelect"),a=parseInt(t?.value||"0",10);if(!a){alert("Please select a staff member to share details with."),t?.focus();return}const i=this.staffList.find(p=>p.id===a);if(!i){alert("Selected staff member is invalid.");return}const n=document.getElementById("usmFollowupType")?.value||"secondary_followup",s=document.getElementById("usmNotes")?.value.trim()||"",l=document.getElementById("usmAssignCheck")?.checked||!1,o=this.currentOptions;if(!o)return;const d=document.getElementById("usmCombinedBtn"),r=document.getElementById("usmCrmOnlyBtn"),u=document.getElementById("usmWaOnlyBtn");[d,r,u].forEach(p=>{p&&(p.disabled=!0)});try{let p="",m="";if(e==="crm_only"||e==="combined"||l){const w={target_staff_id:a,followup_type:n,notes:s,assign_as_secondary:l},S=await x.post(`/crm/leads/${o.leadId}/share-details`,w);if(S&&S.success)p=S.wa_url||"",m=S.wa_message||"",o.onShared&&o.onShared(S);else throw new Error(S?.detail||S?.message||"Failed to assign lead in CRM")}if(e==="wa_only"||e==="combined"){if(!p){const w=(i.phone||"").replace(/\D/g,"").slice(-10);if(!w)alert(`Colleague ${i.name} does not have a valid mobile number for WhatsApp.`);else{const S=document.getElementById("usmMessagePreview")?.textContent||"";p=`https://wa.me/91${w}?text=${encodeURIComponent(S)}`}}p&&window.open(p,"_blank")}const g=e==="crm_only"?`Lead successfully assigned to ${i.name} in CRM!`:e==="wa_only"?`WhatsApp launched for ${i.name}!`:`Lead assigned in CRM and WhatsApp opened for ${i.name}!`;alert(`✅ ${g}`),this.close()}catch(p){console.error("[UnifiedShareLeadModal] Share failed:",p),alert(`⚠️ Could not complete lead share: ${p.message||p}`)}finally{[d,r,u].forEach(p=>{p&&(p.disabled=!1)})}}}const ke=new ne;class q{static open(e){$.open(e)}static close(){$.close()}currentReview=null;onSavedCallback=null;scores={script_adherence_score:5,tone_attitude_score:5,info_accuracy_score:5,objection_handling_score:5,closing_technique_score:5,disposition_accuracy_score:5};async open(e){this.injectDOM(),this.onSavedCallback=e.onSaved||null;const t=document.getElementById("mobileUqrRoot"),a=document.getElementById("mobileUqrContent");if(!(!t||!a)){t.style.display="flex",a.innerHTML=`
    ...
    `;try{const i=await x.post("/api/v1/call-quality/reviews/open-or-create",{call_log_id:Number(e.callLogId)||null,call_session_id:e.callSessionId||null,phone:e.phone||null,lead_id:Number(e.leadId)||null});if(!i||!i.success){a.innerHTML=`
    ...
    `,e.addEventListener("click",t=>{t.target===e&&this.close()}),document.body.appendChild(e),document.getElementById("mobileUqrCloseTopBtn")?.addEventListener("click",()=>this.close())}escape(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}}const $=new q;typeof window<"u"&&(window.openUniversalQualityReview=c=>$.open(c),window.closeUniversalQualityReview=()=>$.close());class se{static open(e){N.open(e)}static close(){N.close()}currentEntity=null;activeTab="calls";changesSubfilter="all";activeAudio=null;cache={calls:null,messages:null,changes:{}};open(e){if(!e||!e.entityId&&!e.leadId&&!e.memberId&&!e.phone){console.warn("[UniversalLeadHistoryModal] Invalid options:",e);return}this.injectDOM(),this.stopAudio();const t=e.entityType||(e.memberId?"vgk_member":"crm_lead"),a=parseInt(String(e.entityId||e.leadId||e.memberId||0),10)||0;this.currentEntity={entityType:t,entityId:a,name:e.name||"",phone:e.phone||"",category:e.category||"",status:e.status||"",stage:e.stage||"",assignedTo:e.assignedTo||""},this.cache={calls:null,messages:null,changes:{}},this.activeTab="calls",this.changesSubfilter="all",this.populateHeader();const i=document.getElementById("mobileUhmRoot");i&&(i.style.display="flex"),this.switchTab("calls"),this.fetchSummary()}close(){this.stopAudio();const e=document.getElementById("mobileUhmRoot");e&&(e.style.display="none")}switchTab(e){this.activeTab=e,this.stopAudio(),["Calls","Messages","Changes"].forEach(a=>{const i=document.getElementById("muhmTab"+a);i&&(a.toLowerCase()===e.toLowerCase()?i.classList.add("active"):i.classList.remove("active"))});const t=document.getElementById("muhmSubfilterBar");t&&(t.style.display=e==="changes"?"flex":"none"),this.loadTabData(e)}setSubfilter(e){this.changesSubfilter=e,document.querySelectorAll("#muhmSubfilterBar .muhm-subchip").forEach(a=>{a.classList.remove("active");const i=(a.textContent||"").toLowerCase();(i.includes(e.toLowerCase())||e==="all"&&i.includes("all")||e==="audit"&&i.includes("field"))&&a.classList.add("active")}),this.loadTabData("changes")}stopAudio(){if(this.activeAudio){try{this.activeAudio.pause()}catch{}this.activeAudio=null}document.querySelectorAll("#mobileUhmRoot audio").forEach(t=>{try{t.pause()}catch{}})}populateHeader(){const e=document.getElementById("muhmName");e&&(e.textContent=this.currentEntity.name||(this.currentEntity.entityType==="vgk_member"?"Channel Partner":`Lead #${this.currentEntity.entityId}`));const t=document.getElementById("muhmBadge");t&&(this.currentEntity.entityType==="vgk_member"?(t.textContent="VGK Member",t.className="muhm-badge muhm-badge-vgk"):(t.textContent="CRM Lead",t.className="muhm-badge muhm-badge-lead"));const a=document.getElementById("muhmPhone");a&&(a.textContent=this.currentEntity.phone||"—");const i=document.getElementById("muhmMeta");if(i){const n=[];this.currentEntity.category&&n.push(this.currentEntity.category),(this.currentEntity.stage||this.currentEntity.status)&&n.push(this.currentEntity.stage||this.currentEntity.status),this.currentEntity.assignedTo&&n.push(`Assigned: ${this.currentEntity.assignedTo}`),i.textContent=n.join(" • ")||"—"}["Calls","Messages","Changes"].forEach(n=>{const s=document.getElementById("muhmCount"+n);s&&(s.textContent="…")})}async fetchSummary(){try{const e=await x.get(`/api/v1/crm/universal-history/entity-summary?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}`);if(e&&e.success&&e.entity){const t=document.getElementById("muhmName");t&&e.entity.name&&(t.textContent=e.entity.name);const a=document.getElementById("muhmPhone");if(a&&(a.textContent=e.entity.display_phone||e.entity.primary_phone||"—"),e.counts){const i=document.getElementById("muhmCountCalls");i&&(i.textContent=String(e.counts.calls||0));const n=document.getElementById("muhmCountMessages");n&&(n.textContent=String(e.counts.messages||0));const s=document.getElementById("muhmCountChanges");s&&(s.textContent=String(e.counts.changes||0))}}}catch(e){console.warn("[UniversalHistory] Failed to fetch mobile entity summary:",e)}}async loadTabData(e){const t=document.getElementById("muhmContent");if(!(!t||!this.currentEntity)){if(e==="calls"){const a=i=>{t.innerHTML=this.renderCalls(i),t.querySelectorAll(".muhm-review-call-btn").forEach(n=>{n.addEventListener("click",s=>{const l=s.currentTarget,o=Number(l.dataset.callid)||0,d=l.dataset.sessionid||"",r=l.dataset.phone||"",u=Number(l.dataset.leadid)||0;q.open({callLogId:o,callSessionId:d,phone:r,leadId:u,onSaved:()=>{this.cache.calls=null,this.loadTabData("calls")}})})})};if(this.cache.calls){a(this.cache.calls.items);return}t.innerHTML='<div class="muhm-loading">Loading calls & recordings…</div>';try{const i=await x.get(`/api/v1/crm/universal-history/calls?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&limit=100`);if(i&&i.success){this.cache.calls=i;const n=document.getElementById("muhmCountCalls");n&&(n.textContent=String(i.total_calls||0)),a(i.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(i?.detail||"Failed to load calls")}</div>`}catch(i){t.innerHTML=`<div class="muhm-empty">Error loading calls: ${this.escape(i.message)}</div>`}}else if(e==="messages"){if(this.cache.messages){t.innerHTML=this.renderMessages(this.cache.messages.items);return}t.innerHTML='<div class="muhm-loading">Loading messages & chat…</div>';try{const a=await x.get(`/api/v1/crm/universal-history/messages?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&limit=100`);if(a&&a.success){this.cache.messages=a;const i=document.getElementById("muhmCountMessages");i&&(i.textContent=String(a.total_messages||0)),t.innerHTML=this.renderMessages(a.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(a?.detail||"Failed to load messages")}</div>`}catch(a){t.innerHTML=`<div class="muhm-empty">Error loading messages: ${this.escape(a.message)}</div>`}}else if(e==="changes"){const a=this.changesSubfilter||"all";if(this.cache.changes[a]){t.innerHTML=this.renderChanges(this.cache.changes[a].items);return}t.innerHTML='<div class="muhm-loading">Loading change history…</div>';try{const i=await x.get(`/api/v1/crm/universal-history/changes?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&subfilter=${a}&limit=100`);if(i&&i.success){if(this.cache.changes[a]=i,a==="all"){const n=document.getElementById("muhmCountChanges");n&&(n.textContent=String(i.total_changes||0))}t.innerHTML=this.renderChanges(i.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(i?.detail||"Failed to load changes")}</div>`}catch(i){t.innerHTML=`<div class="muhm-empty">Error loading changes: ${this.escape(i.message)}</div>`}}}}renderCalls(e){return!e||!e.length?'<div class="muhm-empty">No call records or recordings found.</div>':e.map(t=>{(t.direction||"").toLowerCase();const a=this.formatDuration(t.duration_seconds);let i="";t.has_recording&&t.recording_url&&(i=`
        ...
        `);const n=t.id&&String(t.id).startsWith("scl_")&&t.raw_id||0,s=t.call_session_id||"",l=this.escape(t.phone||this.currentEntity&&this.currentEntity.phone||""),o=this.currentEntity&&this.currentEntity.entityId?this.currentEntity.entityId:0;let d="";t.quality_score!=null&&t.quality_score!==void 0&&(d=`<span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;display:inline-flex;align-items:center;">★ ${t.quality_score}% QA</span>`);const r=!!(t.has_recording&&t.recording_url);let u="";return(r||t.quality_score!=null)&&(u=`
          <button class="muhm-review-call-btn" data-callid="${n}" data-sessionid="${this.escape(s)}" data-phone="${l}" data-leadid="${o}" style="background:#fef3c7;color:#92400e;border:1px solid #fde68a;border-radius:4px;padding:2px 8px;font-size:10.5px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;">
        ...
        <div class="muhm-card">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div>
              ...
              <div style="font-size:11px;color:#64748b;margin-top:2px">${this.formatDate(t.timestamp)} • ${this.escape(t.staff_name||"Staff")}</div>
            </div>
            <div style="text-align:right">
              <span class="muhm-tag">${this.escape(t.source||"Call")}</span>
      ...
      `}).join("")}renderMessages(e){return!e||!e.length?'<div class="muhm-empty">No messages or chat history found.</div>':'<div style="display:flex;flex-direction:column;gap:12px">'+e.map(t=>{const a=(t.direction||"").toLowerCase()==="outbound",i=a?"outbound":"inbound";let n="";const s=t.media_url||"",l=(t.media_type||"").toLowerCase(),o=t.media_name||"Attachment",d=(t.media_mime_type||"").toLowerCase();if(s){const g=l==="document"&&(o.toLowerCase().endsWith(".pdf")||d.includes("pdf"))||s.toLowerCase().includes(".pdf"),w=l==="image"||d.startsWith("image/")||/\.(jpg|jpeg|png|webp|gif)(\?.*)?$/i.test(s),S=l==="audio"||d.startsWith("audio/")||/\.(mp3|ogg|wav|aac|m4a)(\?.*)?$/i.test(s);w?n=`
            ...
            `}const r=(t.status||"").toLowerCase();let u="";a?r==="failed"?u=`<span class="muhm-status-failed" title="${this.escape(t.failure_reason||t.error_message||"Delivery failed")}">⚠ Failed${t.error_code?" ("+this.escape(t.error_code)+")":""}</span>`:r==="read"?u=`<span class="muhm-status-badge" style="color:#0284c7" title="Read ${this.formatDate(t.read_at||t.timestamp)}"><span style="letter-spacing:-1px">✓✓</span> Read</span>`:r==="delivered"?u=`<span class="muhm-status-badge" style="color:#64748b" title="Delivered ${this.formatDate(t.delivered_at||t.timestamp)}"><span style="letter-spacing:-1px">✓✓</span> Delivered</span>`:r==="queued"?u='<span class="muhm-status-badge" style="color:#d97706" title="Queued">⏳ Queued</span>':u='<span class="muhm-status-badge" style="color:#64748b" title="Sent">✓ Sent</span>':u='<span class="muhm-status-badge" style="color:#059669" title="Received">↙ Received</span>';let p="";a&&r==="failed"&&(t.failure_reason||t.error_message)&&(p=`<div class="muhm-error-snippet">ℹ ${this.escape(t.failure_reason||t.error_message)}</div>`);const m=t.message_text?`<div style="white-space:pre-wrap">${this.escape(t.message_text)}</div>`:"";return`
        ...
        `}).join("")+"</div>"}renderChanges(e){return!e||!e.length?'<div class="muhm-empty">No change history or notes found.</div>':'<div style="padding-left:14px;border-left:2px solid #e2e8f0;display:flex;flex-direction:column;gap:12px">'+e.map(t=>{let a="";return(t.old_val||t.new_val)&&(a=`
            <div style="font-size:11px;margin-top:3px">
              ${t.old_val?`<span style="color:#ef4444;text-decoration:line-through;margin-right:4px">${this.escape(t.old_val)}</span> → `:""}
              <span style="color:#10b981;font-weight:600">${this.escape(t.new_val||"")}</span>
            </div>
          `),`
          <div style="position:relative">
            ...
            <div class="muhm-card" style="margin:0">
              <div style="display:flex;justify-content:space-between;font-size:12.5px;font-weight:600;color:#0f172a">
                <span>${this.escape(t.title||t.event_type||"Change")}</span>
                <span style="font-size:10.5px;color:#94a3b8;font-weight:normal">${this.formatDate(t.timestamp)}</span>
              </div>
              ${t.details?`<div style="font-size:11.5px;color:#475569;margin-top:3px">${this.escape(t.details)}</div>`:""}
              ${a}
              <div style="font-size:10.5px;color:#94a3b8;margin-top:4px">By: ${this.escape(t.author_name||"Staff")}</div>
            </div>
          </div>
