    ...
    `,e.addEventListener("click",t=>{t.target===e&&this.close()}),document.body.appendChild(e),document.getElementById("mobileUqrCloseTopBtn")?.addEventListener("click",()=>this.close())}escape(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}}const V=new Y;typeof window<"u"&&(window.openUniversalQualityReview=p=>V.open(p),window.closeUniversalQualityReview=()=>V.close());class de{static open(e){U.open(e)}static close(){U.close()}currentEntity=null;activeTab="calls";changesSubfilter="all";activeAudio=null;cache={calls:null,messages:null,changes:{}};open(e){if(!e||!e.entityId&&!e.leadId&&!e.memberId&&!e.phone){console.warn("[UniversalLeadHistoryModal] Invalid options:",e);return}this.injectDOM(),this.stopAudio();const t=e.entityType||(e.memberId?"vgk_member":"crm_lead"),a=parseInt(String(e.entityId||e.leadId||e.memberId||0),10)||0;this.currentEntity={entityType:t,entityId:a,name:e.name||"",phone:e.phone||"",category:e.category||"",status:e.status||"",stage:e.stage||"",assignedTo:e.assignedTo||""},this.cache={calls:null,messages:null,changes:{}},this.activeTab="calls",this.changesSubfilter="all",this.populateHeader();const i=document.getElementById("mobileUhmRoot");i&&(i.style.display="flex"),this.switchTab("calls"),this.fetchSummary()}close(){this.stopAudio();const e=document.getElementById("mobileUhmRoot");e&&(e.style.display="none")}switchTab(e){this.activeTab=e,this.stopAudio(),["Calls","Messages","Changes"].forEach(a=>{const i=document.getElementById("muhmTab"+a);i&&(a.toLowerCase()===e.toLowerCase()?i.classList.add("active"):i.classList.remove("active"))});const t=document.getElementById("muhmSubfilterBar");t&&(t.style.display=e==="changes"?"flex":"none"),this.loadTabData(e)}setSubfilter(e){this.changesSubfilter=e,document.querySelectorAll("#muhmSubfilterBar .muhm-subchip").forEach(a=>{a.classList.remove("active");const i=(a.textContent||"").toLowerCase();(i.includes(e.toLowerCase())||e==="all"&&i.includes("all")||e==="audit"&&i.includes("field"))&&a.classList.add("active")}),this.loadTabData("changes")}stopAudio(){if(this.activeAudio){try{this.activeAudio.pause()}catch{}this.activeAudio=null}document.querySelectorAll("#mobileUhmRoot audio").forEach(t=>{try{t.pause()}catch{}})}populateHeader(){const e=document.getElementById("muhmName");e&&(e.textContent=this.currentEntity.name||(this.currentEntity.entityType==="vgk_member"?"Channel Partner":`Lead #${this.currentEntity.entityId}`));const t=document.getElementById("muhmBadge");t&&(this.currentEntity.entityType==="vgk_member"?(t.textContent="VGK Member",t.className="muhm-badge muhm-badge-vgk"):(t.textContent="CRM Lead",t.className="muhm-badge muhm-badge-lead"));const a=document.getElementById("muhmPhone");a&&(a.textContent=this.currentEntity.phone||"—");const i=document.getElementById("muhmMeta");if(i){const n=[];this.currentEntity.category&&n.push(this.currentEntity.category),(this.currentEntity.stage||this.currentEntity.status)&&n.push(this.currentEntity.stage||this.currentEntity.status),this.currentEntity.assignedTo&&n.push(`Assigned: ${this.currentEntity.assignedTo}`),i.textContent=n.join(" • ")||"—"}["Calls","Messages","Changes"].forEach(n=>{const s=document.getElementById("muhmCount"+n);s&&(s.textContent="…")})}async fetchSummary(){try{const e=await k.get(`/api/v1/crm/universal-history/entity-summary?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}`);if(e&&e.success&&e.entity){const t=document.getElementById("muhmName");t&&e.entity.name&&(t.textContent=e.entity.name);const a=document.getElementById("muhmPhone");if(a&&(a.textContent=e.entity.display_phone||e.entity.primary_phone||"—"),e.counts){const i=document.getElementById("muhmCountCalls");i&&(i.textContent=String(e.counts.calls||0));const n=document.getElementById("muhmCountMessages");n&&(n.textContent=String(e.counts.messages||0));const s=document.getElementById("muhmCountChanges");s&&(s.textContent=String(e.counts.changes||0))}}}catch(e){console.warn("[UniversalHistory] Failed to fetch mobile entity summary:",e)}}async loadTabData(e){const t=document.getElementById("muhmContent");if(!(!t||!this.currentEntity)){if(e==="calls"){const a=i=>{t.innerHTML=this.renderCalls(i),t.querySelectorAll(".muhm-review-call-btn").forEach(n=>{n.addEventListener("click",s=>{const l=s.currentTarget,o=Number(l.dataset.callid)||0,d=l.dataset.sessionid||"",r=l.dataset.phone||"",u=Number(l.dataset.leadid)||0;Y.open({callLogId:o,callSessionId:d,phone:r,leadId:u,onSaved:()=>{this.cache.calls=null,this.loadTabData("calls")}})})})};if(this.cache.calls){a(this.cache.calls.items);return}t.innerHTML='<div class="muhm-loading">Loading calls & recordings…</div>';try{const i=await k.get(`/api/v1/crm/universal-history/calls?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&limit=100`);if(i&&i.success){this.cache.calls=i;const n=document.getElementById("muhmCountCalls");n&&(n.textContent=String(i.total_calls||0)),a(i.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(i?.detail||"Failed to load calls")}</div>`}catch(i){t.innerHTML=`<div class="muhm-empty">Error loading calls: ${this.escape(i.message)}</div>`}}else if(e==="messages"){if(this.cache.messages){t.innerHTML=this.renderMessages(this.cache.messages.items);return}t.innerHTML='<div class="muhm-loading">Loading messages & chat…</div>';try{const a=await k.get(`/api/v1/crm/universal-history/messages?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&limit=100`);if(a&&a.success){this.cache.messages=a;const i=document.getElementById("muhmCountMessages");i&&(i.textContent=String(a.total_messages||0)),t.innerHTML=this.renderMessages(a.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(a?.detail||"Failed to load messages")}</div>`}catch(a){t.innerHTML=`<div class="muhm-empty">Error loading messages: ${this.escape(a.message)}</div>`}}else if(e==="changes"){const a=this.changesSubfilter||"all";if(this.cache.changes[a]){t.innerHTML=this.renderChanges(this.cache.changes[a].items);return}t.innerHTML='<div class="muhm-loading">Loading change history…</div>';try{const i=await k.get(`/api/v1/crm/universal-history/changes?entity_type=${this.currentEntity.entityType}&entity_id=${this.currentEntity.entityId||0}&phone=${encodeURIComponent(this.currentEntity.phone||"")}&name=${encodeURIComponent(this.currentEntity.name||"")}&subfilter=${a}&limit=100`);if(i&&i.success){if(this.cache.changes[a]=i,a==="all"){const n=document.getElementById("muhmCountChanges");n&&(n.textContent=String(i.total_changes||0))}t.innerHTML=this.renderChanges(i.items)}else t.innerHTML=`<div class="muhm-empty">${this.escape(i?.detail||"Failed to load changes")}</div>`}catch(i){t.innerHTML=`<div class="muhm-empty">Error loading changes: ${this.escape(i.message)}</div>`}}}}renderCallFromBadge(e){const t=(e.dialed_page||e.call_from||e.source||"").trim(),a=t.toLowerCase(),i=(e.device_call_id||e.call_session_id||"").toLowerCase(),n=(e.call_type||e.direction||"").toUpperCase();return a.includes("auto")||a.includes("dialer")||i.includes("cda_")||n==="DIALER"?'<span style="background:#f5f3ff;color:#6b21a8;border:1px solid #ddd6fe;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-robot" style="color:#6b21a8;margin-right:4px;"></i>Auto Dialer</span>':a.includes("my lead")||a==="my leads"?'<span style="background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-user-check" style="color:#1d4ed8;margin-right:4px;"></i>My Leads</span>':a.includes("staff lead")||a==="staff leads"?'<span style="background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-users" style="color:#15803d;margin-right:4px;"></i>Staff Leads</span>':a.includes("crm dashboard")||a.includes("dashboard")?'<span style="background:#fdf2f8;color:#be185d;border:1px solid #fbcfe8;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-chart-line" style="color:#be185d;margin-right:4px;"></i>CRM Dashboard</span>':a.includes("softphone center")||a.includes("softphone hub")||a==="softphone"||a==="plivo webrtc"?'<span style="background:#e0f2fe;color:#0369a1;border:1px solid #bae6fd;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-headset" style="color:#0369a1;margin-right:4px;"></i>Softphone Center</span>':a.includes("operator")?'<span style="background:#fffbeb;color:#b45309;border:1px solid #fde68a;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-head-side-headphones" style="color:#b45309;margin-right:4px;"></i>Operator Calls</span>':a.includes("whatsapp")?'<span style="background:#f0fdf4;color:#16a34a;border:1px solid #86efac;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fab fa-whatsapp" style="color:#16a34a;margin-right:4px;"></i>WhatsApp Center</span>':a.includes("planner")?'<span style="background:#faf5ff;color:#7e22ce;border:1px solid #e9d5ff;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-calendar-day" style="color:#7e22ce;margin-right:4px;"></i>Day Planner</span>':a.includes("task")?'<span style="background:#f1f5f9;color:#334155;border:1px solid #cbd5e1;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-tasks" style="color:#334155;margin-right:4px;"></i>Tasks</span>':a.includes("master")?'<span style="background:#fef3c7;color:#92400e;border:1px solid #fcd34d;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-database" style="color:#92400e;margin-right:4px;"></i>Master Leads</span>':a.includes("bank")?'<span style="background:#ecfdf5;color:#065f46;border:1px solid #a7f3d0;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-landmark" style="color:#065f46;margin-right:4px;"></i>Bank Wise Leads</span>':a.includes("inbound")||a.includes("did")||n==="INCOMING"||n==="INBOUND"?'<span style="background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-phone-arrow-down-left" style="color:#047857;margin-right:4px;"></i>Inbound DID</span>':a.includes("mobile")||a.includes("sim")||a.includes("native")?'<span style="background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-sim-card" style="color:#047857;margin-right:4px;"></i>Native SIM</span>':`<span style="background:#f8fafc;color:#475569;border:1px solid #cbd5e1;font-size:10.5px;font-weight:600;padding:2px 6px;border-radius:4px;display:inline-flex;align-items:center;"><i class="fas fa-file-alt" style="margin-right:4px;color:#94a3b8;"></i>${this.escape(t||"My Leads")}</span>`}renderCalls(e){return!e||!e.length?'<div class="muhm-empty">No call records or recordings found.</div>':e.map(t=>{(t.direction||"").toLowerCase();const a=this.formatDuration(t.duration_seconds),i=Number(t.duration_seconds||0);let n="";t.has_recording&&t.recording_url?n=`
      ...
      `}).join("")}renderMessages(e){return!e||!e.length?'<div class="muhm-empty">No messages or chat history found.</div>':'<div style="display:flex;flex-direction:column;gap:12px">'+e.map(t=>{const a=(t.direction||"").toLowerCase()==="outbound",i=a?"outbound":"inbound";let n="";const s=t.media_url||"",l=(t.media_type||"").toLowerCase(),o=t.media_name||"Attachment",d=(t.media_mime_type||"").toLowerCase();if(s){const g=l==="document"&&(o.toLowerCase().endsWith(".pdf")||d.includes("pdf"))||s.toLowerCase().includes(".pdf"),b=l==="image"||d.startsWith("image/")||/\.(jpg|jpeg|png|webp|gif)(\?.*)?$/i.test(s),v=l==="audio"||d.startsWith("audio/")||/\.(mp3|ogg|wav|aac|m4a)(\?.*)?$/i.test(s);b?n=`
        ...
        `}).join("")+"</div>"}renderChanges(e){if(!e||!e.length)return'<div class="muhm-empty">No change history or notes found.</div>';const t=[],a={};return e.forEach(i=>{let n="General History";if(i.timestamp)try{const l=new Date(i.timestamp);isNaN(l.getTime())||(n=l.toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}))}catch{}const s=n;a[s]||(a[s]={dateKey:n,items:[]},t.push(a[s])),a[s].items.push(i)}),'<div style="padding-left:14px;border-left:2px solid #e2e8f0;display:flex;flex-direction:column;gap:12px">'+t.map(i=>{let n="";return i.items.forEach(s=>{let l="";(s.old_val||s.new_val)&&(l=`
            ...
            `);let T="";o?T=`<span style="font-size:10px;font-weight:700;color:#15803d;background:#dcfce7;padding:1px 5px;border-radius:4px;">Handled by: ${this.escape(s.handled_by||s.author_name||"Staff")}</span>`:T=`<span style="font-size:10px;font-weight:600;color:#475569;background:#e2e8f0;padding:1px 5px;border-radius:4px;">By: ${this.escape(s.author_name||s.handled_by||"Staff")}</span>`;let R="";if(s.timestamp)try{const _=new Date(s.timestamp);isNaN(_.getTime())||(R=_.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"}))}catch{}n+=`
            <div style="background:${c};border:1px solid ${m};border-radius:6px;padding:8px 10px;margin-bottom:6px;">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:4px;flex-wrap:wrap;">
                ...
                <div style="display:flex;align-items:center;gap:4px;">
                  ${T}
                  ${R?`<span style="font-size:10px;color:#94a3b8">${this.escape(R)}</span>`:""}
                </div>
              </div>
