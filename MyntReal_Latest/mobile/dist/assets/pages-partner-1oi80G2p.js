import{d as k,b,t as _,a as f}from"./services-BmWKyE03.js";import{p as L,P as h,M as $,b as q}from"./components-CNELqz87.js";class H{container;user=null;data=null;loading=!0;constructor(e){this.container=e}async init(){const e=k.getAuthState();this.user=e.user,L.setUser(this.user),this.render(),await this.loadDashboard()}async loadDashboard(){this.loading=!0,this.updateContent();try{const e=await b.get("/partner/dashboard/stats");e.success&&e.data&&(this.data=e.data)}catch(e){console.error("[PartnerDashboard] Failed to load:",e)}this.loading=!1,this.updateContent()}render(){const e=this.user?.name||this.user?.partner_name||"Partner",t=this.user?.partner_code||this.user?.partner_id||"PARTNER",a=this.user?.partner_type||this.user?.type||"Official Partner",i=e.split(" ").map(o=>o[0]).join("").toUpperCase().slice(0,2);this.container.innerHTML=`
      <style>
    ...
    `,e.querySelectorAll("[data-page]").forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-page");i&&_.navigate(i)})})}}class j{container;orders=[];pendingInvoices=[];loading=!0;pendingLoading=!1;pendingLoaded=!1;filter="all";activeTab="orders";constructor(e){this.container=e}async init(){this.render(),await this.loadOrders()}async loadOrders(){this.loading=!0,this.updateContent();try{const e=await b.get("/partner/orders");e.success&&e.data&&(this.orders=e.data.orders||[])}catch(e){console.error("[PartnerOrders] Failed to load:",e)}this.loading=!1,this.updateContent()}async loadPendingDelivery(){this.pendingLoading=!0,this.updateContent();try{const e=await b.get("/partner/pending-dispatch");e.success&&e.data&&(this.pendingInvoices=e.data.invoices||[])}catch(e){console.error("[PartnerOrders] Pending delivery load failed:",e)}this.pendingLoading=!1,this.pendingLoaded=!0,this.updateContent()}render(){this.container.innerHTML=`
    ...
    `,h.attachListeners({title:"My Orders",showBack:!0})}updateContent(){const e=document.getElementById("pageContent");if(!e)return;const t=`
      <div style="display:flex;border-bottom:2px solid rgba(255,255,255,0.1);margin-bottom:14px">
        ...
        <button id="tabPending" style="flex:1;padding:10px;background:none;border:none;border-bottom:3px solid ${this.activeTab==="pending"?"#10b981":"transparent"};color:${this.activeTab==="pending"?"#10b981":"#8892b0"};font-size:13px;font-weight:600;cursor:pointer;margin-bottom:-2px"><i class="fas fa-truck" style="margin-right:4px"></i>Pending Delivery</button>
          ...
          ${a}`}e.querySelector("#tabOrders")?.addEventListener("click",()=>{this.activeTab="orders",this.updateContent()}),e.querySelector("#tabPending")?.addEventListener("click",()=>{this.activeTab="pending",!this.pendingLoaded&&!this.pendingLoading?this.loadPendingDelivery():this.updateContent()})}formatDate(e){return e?new Date(e).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):"—"}}class O{container;invoices=[];sfmsInvoices=[];loading=!0;sfmsLoading=!1;sfmsLoaded=!1;activeTab="invoices";constructor(e){this.container=e}async init(){this.render(),await this.loadInvoices()}async loadInvoices(){this.loading=!0,this.updateContent();try{const e=await b.get("/partner/invoices");e.success&&e.data&&(this.invoices=e.data.invoices||[])}catch(e){console.error("[PartnerInvoices] Failed to load:",e)}this.loading=!1,this.updateContent()}async loadSfmsInvoices(){this.sfmsLoading=!0,this.updateContent();try{const e=await b.get("/partner/sfms-invoices?per_page=50");e.success&&e.data&&(this.sfmsInvoices=e.data.data||[])}catch(e){console.error("[PartnerInvoices] SFMS load failed:",e)}this.sfmsLoading=!1,this.sfmsLoaded=!0,this.updateContent()}render(){this.container.innerHTML=`
      <div class="page-container">
    ...
    `,h.attachListeners({title:"Invoices",showBack:!0})}updateContent(){const e=document.getElementById("pageContent");if(!e)return;if(this.loading){e.innerHTML='<div class="loading-state">Loading...</div>';return}const t=`
      <div style="display:flex;border-bottom:2px solid rgba(255,255,255,0.1);margin-bottom:14px">
        ...
        <div class="list-container">
          ...
          </div>`;document.getElementById("tabInvoices")?.addEventListener("click",()=>{this.activeTab="invoices",this.updateContent()}),document.getElementById("tabSfms")?.addEventListener("click",()=>{this.activeTab="sfms",!this.sfmsLoaded&&!this.sfmsLoading?this.loadSfmsInvoices():this.updateContent()})}formatDate(e){return e?new Date(e).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):"—"}}class U{container;data=null;loading=!0;constructor(e){this.container=e}async init(){this.render(),await this.loadRevenue()}async loadRevenue(){this.loading=!0,this.updateContent();try{const e=await b.get("/partner/revenue-dashboard");e.success&&e.data&&(this.data=e.data)}catch(e){console.error("[PartnerRevenue] Failed to load:",e)}this.loading=!1,this.updateContent()}render(){this.container.innerHTML=`
      <div class="page-container">
    ...
    `,h.attachListeners({title:"Service Tickets",showBack:!0})}updateContent(){const e=document.getElementById("pageContent");if(!e)return;if(this.loading){e.innerHTML='<div class="loading-state">Loading tickets...</div>';return}const t=`
      ...
      </div>`;if(this.activeTab==="tickets"){const a=this.tickets.filter(n=>n.status.toLowerCase()==="open").length,i=this.tickets.filter(n=>n.status.toLowerCase()==="in progress"||n.status.toLowerCase()==="in-progress").length,o=this.tickets.filter(n=>n.status.toLowerCase()==="closed"||n.status.toLowerCase()==="resolved").length,r=new $({columns:[{key:"ticket_number",label:"Ticket #",render:n=>`<span style="color: #64d2ff; font-weight: 600;">${n}</span>`},{key:"subject",label:"Subject"},{key:"category",label:"Category"},{key:"priority",label:"Priority",render:n=>this.getPriorityBadge(n)},{key:"status",label:"Status",render:n=>this.getStatusBadge(n)},{key:"created_at",label:"Created",render:n=>this.formatDate(n)}],data:this.tickets,emptyMessage:"No tickets found. Raise a new ticket to get started."});e.innerHTML=t+`
      ...
      <div class="partner-leads-page">
        ...
        <div class="leads-content">
          <div class="status-tabs">
            ...
            ${A.map(a=>`<button class="status-tab" data-status="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</button>`).join("")}
          </div>
    ...
    `,this.attachEventListeners(),this.updateContent()}attachEventListeners(){document.getElementById("menuBtn")?.addEventListener("click",()=>L.toggle()),document.getElementById("addLeadBtn")?.addEventListener("click",()=>this.showAddLeadModal()),document.querySelectorAll(".status-tab").forEach(a=>{a.addEventListener("click",i=>{document.querySelectorAll(".status-tab").forEach(o=>o.classList.remove("active")),i.target.classList.add("active"),this.selectedStatus=i.target.dataset.status||"all",this.updateContent()})});const e=document.getElementById("dateFilter");e&&e.addEventListener("change",()=>{this.dateFilter=e.value,this.loadLeads()});const t=document.getElementById("roleFilter");t&&t.addEventListener("change",()=>{this.roleFilter=t.value,this.loadLeads()})}updateContent(){const e=document.getElementById("leadsList");if(!e)return;if(this.loading){e.innerHTML='<div class="loading-state">Loading leads...</div>';return}const t=this.selectedStatus==="all"?this.leads:this.leads.filter(a=>a.status===this.selectedStatus);if(t.length===0){e.innerHTML=`
      ...
      <div class="lead-card" data-id="${a.id}">
        <div class="lead-card-header">
          <div>
            ...
            <div class="lead-contact-row">
              ${a.is_phone_masked?`
                <span class="lead-phone-link" style="color:#64748b;font-weight:600"><span style="font-size:11px">🛡️</span> ${a.phone||"No phone"}</span>
              `:`
                <a href="tel:${a.phone||""}" class="lead-phone-link" onclick="event.stopPropagation()">${a.phone||"No phone"}</a>
                ${a.phone?`<button class="whatsapp-link open-partner-wa-btn" data-phone="${(a.phone||"").replace(/\D/g,"")}" data-name="${a.name}" data-id="${a.id}" data-cat="${a.category_name||""}" onclick="event.stopPropagation()" style="background:none;border:none;cursor:pointer;padding:0 2px;">💬</button>`:""}
              `}
            </div>
          </div>
        ...
        <div class="lead-actions">
          ${a.is_phone_masked?`
            <button class="action-btn call btn-partner-click-to-call" data-id="${a.id}" onclick="event.stopPropagation()" title="Secure Click-to-Call">📞</button>
          `:`
            <a href="tel:${a.phone||""}" class="action-btn call" onclick="event.stopPropagation()">📞</a>
            <button class="action-btn whatsapp open-partner-wa-btn" data-phone="${(a.phone||"").replace(/\D/g,"")}" data-name="${a.name}" data-id="${a.id}" data-cat="${a.category_name||""}" onclick="event.stopPropagation()" style="border:none;cursor:pointer;" title="Send WhatsApp">💬</button>
          `}
    ...
    `).join(""),document.querySelectorAll(".btn-partner-click-to-call").forEach(a=>{a.addEventListener("click",async i=>{i.preventDefault(),i.stopPropagation();const o=a.dataset.id;if(o&&confirm("Initiate outbound call to customer via secure telephony bridge?"))try{const r=await b.post(`/crm/unified-my-leads/${o}/click-to-call?role=partner`,{});r&&r.success?alert(`📞 Outbound call initiated!
Session: ${r.call_session_id||"—"}
Destination: ${r.destination_phone||"—"}`):alert(`Call initiation failed: ${r?.detail||r?.message||"Unknown error"}`)}catch(r){alert(`Call failed: ${r.message||"Error"}`)}})}),document.querySelectorAll(".open-partner-wa-btn").forEach(a=>{a.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const o=a.dataset.phone||"",r=a.dataset.name||"Customer",n=a.dataset.id||"",s=a.dataset.cat||"";o&&q.open({phone:o,name:r,leadId:n,context:s})})})}showAddLeadModal(){const e=document.createElement("div");e.id="addLeadModal",e.innerHTML=`
      <style>
        #addLeadModal {
    ...
    `,document.body.appendChild(e),e.querySelector(".modal-close")?.addEventListener("click",()=>e.remove()),e.querySelector("#btnCancelLead")?.addEventListener("click",()=>e.remove()),e.querySelector("#btnSaveLead")?.addEventListener("click",()=>this.saveLead(e)),e.addEventListener("click",a=>{a.target===e&&e.remove()});const t=document.getElementById("leadPincode");t&&t.addEventListener("input",()=>{t.value.length===6&&this.lookupPincode(t)})}async saveLead(e){const t=document.getElementById("leadName")?.value?.trim(),a=document.getElementById("leadMobile")?.value?.trim(),i=document.getElementById("leadEmail")?.value?.trim(),o=document.getElementById("leadCategory")?.value,r=document.getElementById("leadPriority")?.value,n=document.getElementById("leadNotes")?.value?.trim(),s=document.getElementById("leadSource")?.value,d=document.getElementById("leadMobileSecondary")?.value?.trim(),g=document.getElementById("leadAddress")?.value?.trim(),p=document.getElementById("leadPhoneWhatsapp")?.checked,m=document.getElementById("leadPhoneSecondaryWhatsapp")?.checked,v=document.getElementById("leadRequirements")?.value?.trim(),u=document.getElementById("leadLookingFor")?.value?.trim(),c=document.getElementById("leadBudgetMin")?.value,y=document.getElementById("leadBudgetMax")?.value,E=document.getElementById("leadArea")?.value?.trim(),B=document.getElementById("leadCity")?.value?.trim(),I=document.getElementById("leadState")?.value?.trim(),T=document.getElementById("leadPincode")?.value?.trim(),z=document.getElementById("leadExpectedCloseDate")?.value,M=document.getElementById("leadNextFollowupDate")?.value,P=document.getElementById("leadTags")?.value?.trim();if(!t||!a){alert("Please enter name and mobile number");return}if(a.length!==10||!/^\d{10}$/.test(a)){alert("Please enter a valid 10-digit mobile number");return}const x=document.getElementById("btnSaveLead");x&&(x.disabled=!0,x.textContent="Creating...");try{const w={name:t,phone:a,email:i||null,category_id:o?parseInt(o):null,priority:r||"medium",status:"new",description:n||null,source:s||"partner_app",phone_primary_whatsapp:p||!1,alternate_phone:d||null,phone_secondary_whatsapp:m||!1,address:g||null,requirements:v||null,looking_for:u||null,budget_min:c?parseFloat(c):null,budget_max:y?parseFloat(y):null,area:E||null,city:B||null,state:I||null,pincode:T||null,expected_close_date:z||null,next_followup_date:M||null,tags:P||null},S=await b.post("/crm/unified-my-leads?role=partner",w);S.success?(e.remove(),alert("Lead added successfully!"),await this.loadLeads()):alert(S.error||"Failed to add lead")}catch(w){console.error("[PartnerLeads] Add lead failed:",w),alert(w.message||"Failed to add lead. Please try again.")}finally{x&&(x.disabled=!1,x.textContent="Create Lead")}}}class Y{container;constructor(e){this.container=e}async init(){this.container.innerHTML=this.renderSkeleton();try{const e=localStorage.getItem("partner_token"),t=localStorage.getItem("partner_company_id")||"",a=f.BASE_SERVER_URL,o=await(await fetch(`${a}/api/v1/partner/auth/me?company_id=${t}`,{headers:{Authorization:`Bearer ${e}`}})).json();o.success||o.partner_code?this.render(o.partner||o):this.renderError("Failed to load profile")}catch{this.renderError("Network error. Please try again.")}}renderSkeleton(){return`
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:50vh;padding:24px;text-align:center">
      ...
      </div>`}}const D={aadhar_front:{label:"Aadhaar — Front",icon:"🪪",color:"#3b82f6",bg:"rgba(59,130,246,.12)"},aadhar_back:{label:"Aadhaar — Back",icon:"🪪",color:"#8b5cf6",bg:"rgba(139,92,246,.12)"},pan_card:{label:"PAN Card",icon:"💳",color:"#f59e0b",bg:"rgba(245,158,11,.12)"},passport_photo:{label:"Passport Photo",icon:"📷",color:"#10b981",bg:"rgba(16,185,129,.12)"}};function C(l){return!l||l==="Not Submitted"?'<span style="background:#1f2937;color:#9ca3af;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">Not Submitted</span>':l==="Approved"?'<span style="background:#064e3b;color:#10b981;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">✓ Approved</span>':l==="Rejected"?'<span style="background:#450a0a;color:#ef4444;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">✗ Rejected</span>':`<span style="background:#451a03;color:#f59e0b;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">⏳ ${l}</span>`}class G{container;constructor(e){this.container=e}async init(){this.container.innerHTML=this.renderSkeleton();try{const e=localStorage.getItem("partner_token")||"",t=localStorage.getItem("partner_company_id")||"",a=f.BASE_SERVER_URL,i={Authorization:`Bearer ${e}`},[o,r,n]=await Promise.all([fetch(`${a}/api/v1/partner/auth/me?company_id=${t}`,{headers:i}),fetch(`${a}/api/v1/partner/auth/my-partnership`,{headers:i}),fetch(`${a}/api/v1/partner/kyc/status`,{headers:i})]);if(!o.ok)throw new Error("Unauthorized");const s=await o.json(),d=s.partner||s,g=r.ok?await r.json():{},p=g.partnership||{},v=(n.ok?await n.json():{}).kyc_documents||{},u=g.kyc_status||"Not Submitted";this.render(d,p,v,u)}catch{this.container.innerHTML=`
        ...
        </div>`}}renderSkeleton(){const e=(t,a="14px")=>`<div style="height:${a};background:#1e293b;border-radius:6px;width:${t};margin-bottom:10px"></div>`;return`<div style="padding:20px">${e("50%","24px")}${e("30%")}${Array(6).fill(e("100%","48px")).join("")}</div>`}render(e,t,a,i){const o=t.days_to_expiry;let r="";if(t.partner_end_date){let s="#064e3b",d="#10b981",g="✅";o!==null&&o<=0?(s="#450a0a",d="#ef4444",g="🔴"):o!==null&&o<=(t.reminder_days_before||90)&&(s="#451a03",d="#f59e0b",g="⚠️");const p=o===null?"":o<=0?`Expired ${Math.abs(o)} day(s) ago`:`${o} day(s) remaining`;r=`
        <div style="background:${s};border-radius:12px;padding:14px 16px;margin-bottom:14px;display:flex;align-items:center;gap:12px">
          ...
          <div>
            ...
            <div style="font-size:12px;color:${d};margin-top:2px">${t.partner_end_date} · ${p}</div>
          </div>
        </div>`}const n=Object.entries(D).map(([s,d])=>{const g=s.replace("aadhar_","aadhaar_"),p=a[g]||a[s]||null,m=p?p.status:"Not Submitted",v=m!=="Approved",u=p?.uploaded_at?new Date(p.uploaded_at).toLocaleDateString("en-IN"):"",c=p?.rejection_reason?`<div style="font-size:10px;color:#ef4444;margin-top:4px;background:#450a0a;padding:3px 8px;border-radius:6px">${p.rejection_reason}</div>`:"";return`
        <div style="display:flex;align-items:center;gap:12px;padding:14px 0;border-bottom:1px solid #1e293b">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:11px;color:#64748b;margin-top:2px">${u?"Uploaded: "+u:"Not yet uploaded"}</div>
            ${c}
          </div>
    ...
    `,window.partnerKYCUpload=s=>{window._currentKYCType=s;const d=document.getElementById("kycUploadInput");d&&d.click()},window.handleKYCFileChange=async s=>{const d=s.files?.[0],g=window._currentKYCType;if(!d||!g)return;if(d.size>5*1024*1024){alert("File too large. Max 5MB.");return}const p=localStorage.getItem("partner_token")||"",m=new FormData;m.append("file",d),m.append("document_type",g);try{const v=f.BASE_SERVER_URL,u=await fetch(`${v}/api/v1/partner/kyc/upload`,{method:"POST",headers:{Authorization:`Bearer ${p}`},body:m}),c=await u.json();c.success||u.ok?(alert("Uploaded successfully! Pending staff review."),this.init()):alert("Upload failed: "+(c.detail||c.message||"Please try again."))}catch{alert("Network error. Please try again.")}s.value=""}}kv(e,t){return`<div style="display:flex;gap:12px;padding:8px 0;border-bottom:1px solid #f3f4f6">
    ...
    </div>`}}function R(l){const e={SUBMITTED:["#451a03","#f59e0b"],ACKNOWLEDGED:["#1e3a5f","#60a5fa"],FULFILLED:["#064e3b","#10b981"],CANCELLED:["#1f2937","#9ca3af"]},[t,a]=e[l]||["#1f2937","#9ca3af"];return`<span style="background:${t};color:${a};padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700">${l}</span>`}class Q{container;cart=new Map;companies=[];activeTab="catalog";searchTimer=null;allItems=[];requests=[];constructor(e){this.container=e}async init(){this.render(),await this.loadCompanies(),await this.loadCatalog()}render(){this.container.innerHTML=`
      <div style="display:flex;flex-direction:column;height:100%;background:#0f172a;">
        ...
        </div>`}).join(""))}async cancelRequest(e){if(!confirm("Cancel this spare parts request?"))return;const t=localStorage.getItem("partner_token")||"",a=f.BASE_SERVER_URL;try{const o=await(await fetch(`${a}/api/v1/partner/auth/spare-requests/${e}/cancel`,{method:"PUT",headers:{Authorization:`Bearer ${t}`}})).json();o.success?(alert("Request cancelled"),this.loadRequests()):alert("Error: "+(o.detail||"Could not cancel"))}catch{alert("Network error")}}}export{H as P,O as a,G as b,K as c,j as d,Y as e,V as f,U as g,W as h,Q as i};
