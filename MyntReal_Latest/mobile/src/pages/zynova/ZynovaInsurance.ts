/**
 * Zynova Insurance Page (VGK Care)
 * DC Protocol: DC_MOBILE_ZYNOVA_INS_001
...
export class ZynovaInsurance {
  private container: HTMLElement;
  private data: ZynovaInsuranceData | null = null;
  ...
  private async loadData(): Promise<void> {
    ...
    try {
      const response = await apiService.get<any>('/users/zynova/insurance');
      if (response.success && response.data) {
        this.data = response.data;
      }
    } catch (error) {
      console.error('[ZynovaInsurance] Failed to load:', error);
    }
  ...
  private render(): void {
    this.container.innerHTML = `
      ...
      <div class="page-container zynova-insurance-page">
        ${PageHeader.render({ title: 'VGK Care - Insurance', showBack: true })}
    ...
    PageHeader.attachListeners({ title: 'VGK Care - Insurance', showBack: true });
  }
