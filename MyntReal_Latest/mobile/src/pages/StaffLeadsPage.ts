...
export class StaffLeadsPage {
  ...
  private updateContent(): void {
    ...
    document.querySelectorAll('.view-lead-detail-btn, .view-lead-name-trigger').forEach(el => {
    ...
    document.querySelectorAll('.open-lead-wa-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    document.querySelectorAll('.open-lead-history-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const el = btn as HTMLElement;
        const leadId = parseInt(el.dataset.id || '0');
        const name = el.dataset.name || 'Lead';
        const phone = el.dataset.phone || '';
        const cat = el.dataset.cat || 'CRM Lead';
        if (leadId) {
    ...
    content.querySelectorAll('.call-softphone-trigger').forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        if (phone) {
          if (leadId) void dialerService.notifyCallActive(parseInt(String(leadId)));
      ...
      <tr data-id="${lead.id}">
        <td class="sticky-col">
          <div class="lead-name-cell">
            <span class="lead-name view-lead-name-trigger" data-id="${lead.id}" style="cursor:pointer;text-decoration:underline dotted;">${lead.name}</span>
        ...
        <td>
          <div class="action-btns">
            <button class="action-btn view view-lead-detail-btn" data-id="${lead.id}" title="View Details">👁</button>
            <button class="action-btn history open-lead-history-btn" data-id="${lead.id}" data-name="${this.escapeAttr(lead.name)}" data-phone="${cleanPhone}" data-cat="${this.escapeAttr(lead.category_name || lead.category || '')}" title="Universal History">🕘</button>
    ...
    return `
      <div class="lead-card card" data-id="${lead.id}">
        <div class="lead-header">
          <div class="lead-info">
            <h4 class="view-lead-name-trigger" data-id="${lead.id}" style="cursor:pointer;">${lead.name}</h4>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.08);">
          <button class="open-lead-history-btn" data-id="${lead.id}" data-name="${this.escapeAttr(lead.name)}" data-phone="${cleanPhone}" data-cat="${this.escapeAttr(lead.category_name || lead.category || '')}" style="background:rgba(99,102,241,0.2);color:#818cf8;border:1px solid rgba(99,102,241,0.4);border-radius:6px;padding:3px 8px;font-size:11px;font-weight:600;display:inline-flex;align-items:center;gap:4px;cursor:pointer;" onclick="event.stopPropagation();">🕘 History</button>
          ${lead.next_followup ? `
            <div class="followup-reminder" style="margin:0;">
              📅 ${new Date(lead.next_followup).toLocaleDateString('en', { day: 'numeric', month: 'short' })}
            </div>
          ` : '<span></span>'}
        </div>
      </div>
  ...
  private async showLeadDetails(leadId: number): Promise<void> {
    ...
    body.innerHTML = `
      ...
      <div class="lead-contact-section">
        ...
        ` : ''}
        <button class="contact-btn open-lead-wa-btn" data-phone="${cleanWaPhone}" data-name="${this.escapeAttr(lead.name)}" data-id="${lead.id}" data-cat="${lead.category || ''}" style="background:none;border:none;cursor:pointer;">
        ...
        <button class="contact-btn open-lead-history-btn" data-id="${lead.id}" data-name="${this.escapeAttr(lead.name)}" data-phone="${cleanWaPhone}" data-cat="${this.escapeAttr(lead.category_name || lead.category || '')}" style="background:rgba(99,102,241,0.15);border:1px solid rgba(99,102,241,0.4);color:#818cf8;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="lead-actions-section">
        <div class="actions-row">
          ...
          <button class="btn btn-secondary" id="detailHistoryLeadBtn" style="background:#4f46e5;color:white;font-weight:700;border:none;border-radius:12px;cursor:pointer;">🕘 History</button>
        </div>
        <div class="actions-row">
    ...
    document.getElementById('detailHistoryLeadBtn')?.addEventListener('click', () => {
      UniversalLeadHistoryModal.open({
        entityType: 'crm_lead',
        entityId: lead.id,
        name: lead.name,
  ...
  private async saveCallDisposition(): Promise<void> {
    ...
    try {
      ...
      // 2. If follow-up date specified, schedule follow-up
      if (fDate) {
