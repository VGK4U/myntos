/**
 * My Announcements — VGK4U Member Parity (Task #34 Phase 2, write-flow)
 * DC Protocol: DC_MOBILE_VGK4U_PARITY_002 (May 2026)
 *
 * Mobile member page for the "My Announcements" write module. Embeds the
 * shared /vgk/my-announcements responsive web view so we inherit form handling,
...
export class VGKMyAnnouncementsPage {
  ...
  static readonly slug = 'my-announcements';
  static readonly label = 'My Announcements';
  static readonly icon = 'fas fa-list';
  static readonly color = '#0d9488';
  static readonly endpoint = '/vgk/my-announcements';
  ...
  async render(): Promise<string> {
    return `
      ${PageHeader.render({ title: 'My Announcements', showBack: true })}
      <div style="padding:14px 12px;background:#f6f9fc;min-height:100vh">
        ...
        <iframe
          ...
          src="${APP_CONFIG.MEDIA_BASE_URL}/vgk/my-announcements?token=${encodeURIComponent(localStorage.getItem("auth_token") || "")}"
  ...
  async afterRender(): Promise<void> {
    /* No-op: the embedded /vgk/my-announcements page handles its own data fetch
       with audience=vgk4u, Zero-Default toggle gate, and full WVV verification. */
  }
}

export default VGKMyAnnouncementsPage;
