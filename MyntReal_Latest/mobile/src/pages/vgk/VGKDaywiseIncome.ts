/**
   * Daywise Income — VGK4U Member Parity (Task #33 Phase 1, read-only)
   * DC Protocol: DC_MOBILE_VGK4U_PARITY_001 (May 2026)
   *
   * Mobile member page for the "Daywise Income" module. Phase 1 is read-only:
   * the page reuses the existing /vgk/daywise-income responsive web view so we
  ...
  export class VGKDaywiseIncomePage {
  ...
  async init(): Promise<void> {
    ...
    static readonly slug = 'daywise-income';
    static readonly label = 'Daywise Income';
    static readonly icon = '📅';
    static readonly color = '#059669';
    /** Backend endpoint (already audience-scoped). Tracked here for Phase 2 native build-out. */
    static readonly endpoint = '/api/v1/admin/daywise-income?audience=vgk4u';

    async render(): Promise<string> {
      return `
        ${PageHeader.render({ title: 'Daywise Income', showBack: true })}
        <div style="padding:14px 12px;background:#f6f9fc;min-height:100vh">
          ...
          <iframe
            ...
            src="${APP_CONFIG.MEDIA_BASE_URL}/vgk/daywise-income?token=${encodeURIComponent(localStorage.getItem("auth_token") || "")}"
    ...
    async afterRender(): Promise<void> {
      // No-op: the embedded /vgk/daywise-income page handles its own data fetch
      // with audience=vgk4u and company-scoped backend filtering.
    }
  }

  export default VGKDaywiseIncomePage;
