/**
   * EV Benefits — VGK4U Member Parity (Task #33 Phase 1, read-only)
   * DC Protocol: DC_MOBILE_VGK4U_PARITY_001 (May 2026)
   *
   * Mobile member page for the "EV Benefits" module. Phase 1 is read-only:
   * the page reuses the existing /vgk/ev-benefits responsive web view so we
  ...
  export class VGKEvBenefitsPage {
  ...
  async init(): Promise<void> {
    ...
    static readonly slug = 'ev-benefits';
    static readonly label = 'EV Benefits';
    ...
    async render(): Promise<string> {
      return `
        ${PageHeader.render({ title: 'EV Benefits', showBack: true })}
        <div style="padding:14px 12px;background:#f6f9fc;min-height:100vh">
          ...
          <iframe
            ...
            src="${APP_CONFIG.MEDIA_BASE_URL}/vgk/ev-benefits?token=${encodeURIComponent(localStorage.getItem("auth_token") || "")}"
    ...
    async afterRender(): Promise<void> {
      // No-op: the embedded /vgk/ev-benefits page handles its own data fetch
      // with audience=vgk4u and company-scoped backend filtering.
    }
  }

  export default VGKEvBenefitsPage;
