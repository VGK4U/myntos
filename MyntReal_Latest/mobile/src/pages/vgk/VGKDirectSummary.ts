/**
   * Direct (L1) — VGK4U Member Parity (Task #33 Phase 1, read-only)
   * DC Protocol: DC_MOBILE_VGK4U_PARITY_001 (May 2026)
   *
   * Mobile member page for the "Direct (L1)" module. Phase 1 is read-only:
   * the page reuses the existing /vgk/direct-summary responsive web view so we
  ...
  export class VGKDirectSummaryPage {
  ...
  async init(): Promise<void> {
    ...
    static readonly slug = 'direct-summary';
    static readonly label = 'Direct (L1)';
    static readonly icon = '①';
    static readonly color = '#475569';
    /** Backend endpoint (already audience-scoped). Tracked here for Phase 2 native build-out. */
    static readonly endpoint = '/api/v1/admin/direct-summary?audience=vgk4u';

    async render(): Promise<string> {
      return `
        ${PageHeader.render({ title: 'Direct (L1)', showBack: true })}
        <div style="padding:14px 12px;background:#f6f9fc;min-height:100vh">
          ...
          <iframe
            ...
            src="${APP_CONFIG.MEDIA_BASE_URL}/vgk/direct-summary?token=${encodeURIComponent(localStorage.getItem("auth_token") || "")}"
    ...
    async afterRender(): Promise<void> {
      // No-op: the embedded /vgk/direct-summary page handles its own data fetch
      // with audience=vgk4u and company-scoped backend filtering.
    }
  }

  export default VGKDirectSummaryPage;
