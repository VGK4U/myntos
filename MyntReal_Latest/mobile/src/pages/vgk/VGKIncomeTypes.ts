/**
   * Income Types — VGK4U Member Parity (Task #33 Phase 1, read-only)
   * DC Protocol: DC_MOBILE_VGK4U_PARITY_001 (May 2026)
   *
   * Mobile member page for the "Income Types" module. Phase 1 is read-only:
   * the page reuses the existing /vgk/income-types responsive web view so we
  ...
  export class VGKIncomeTypesPage {
  ...
  async init(): Promise<void> {
    ...
    static readonly slug = 'income-types';
    static readonly label = 'Income Types';
    static readonly icon = '📊';
    static readonly color = '#2563eb';
    /** Backend endpoint (already audience-scoped). Tracked here for Phase 2 native build-out. */
    static readonly endpoint = '/api/v1/admin/income-types?audience=vgk4u';

    async render(): Promise<string> {
      return `
        ${PageHeader.render({ title: 'Income Types', showBack: true })}
        <div style="padding:14px 12px;background:#f6f9fc;min-height:100vh">
          ...
          <iframe
            ...
            src="${APP_CONFIG.MEDIA_BASE_URL}/vgk/income-types?token=${encodeURIComponent(localStorage.getItem("auth_token") || "")}"
    ...
    async afterRender(): Promise<void> {
      // No-op: the embedded /vgk/income-types page handles its own data fetch
      // with audience=vgk4u and company-scoped backend filtering.
    }
  }

  export default VGKIncomeTypesPage;
