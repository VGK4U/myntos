...
Regards,
${a}${i}
${n} | MyntReal Workflows`}open(e){this.currentOptions=e,this.activeMode="scanned",this.render()}close(){this.modalEl&&(this.modalEl.remove(),this.modalEl=null)}render(){if(this.close(),!this.currentOptions)return;const{phone:e,name:t,context:a,defaultMessage:s}=this.currentOptions,n=(e||"").replace(/\D/g,"").slice(-10),i=this.getSenderSignature(),l=(s||w.greeting.text)+i;this.modalEl=document.createElement("div"),this.modalEl.id="unifiedWAModal",this.modalEl.className="uwa-modal-backdrop",this.modalEl.innerHTML=`
    ...
    `}attachEventListeners(){this.container&&(document.getElementById("mnrDrawerClose")?.addEventListener("click",()=>this.close()),this.container.querySelectorAll("[data-toggle]").forEach(e=>{e.addEventListener("click",t=>{const a=e.dataset.toggle;this.toggleSection(a),t.stopPropagation()})}),this.container.querySelectorAll("[data-route]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.route;g.navigate(t),this.close()})}),document.getElementById("mnrLogoutBtn")?.addEventListener("click",async()=>{confirm("Are you sure you want to logout?")&&(await p.logout(),window.dispatchEvent(new CustomEvent("logout")),this.close())}))}toggleSection(e){this.expandedSections.has(e)?this.expandedSections.delete(e):this.expandedSections.add(e),this.updateUI()}updateUI(){this.container&&(this.container.innerHTML=this.render(),this.attachEventListeners())}open(){this.isOpen||(this.isOpen=!0,this.container?.classList.add("open"),this.overlay?.classList.add("visible"),document.body.style.overflow="hidden")}close(){this.isOpen&&(this.isOpen=!1,this.container?.classList.remove("open"),this.overlay?.classList.remove("visible"),document.body.style.overflow="")}toggle(){this.isOpen?this.close():this.open()}destroy(){this.container?.remove(),this.overlay?.remove()}}const q=new O;class J{config;constructor(e){this.config=e}render(){if(this.config.loading)return`
      ...
      `;const e=s=>this.config.sortColumn===s?this.config.sortDirection==="asc"?" ↑":" ↓":"",t=this.config.columns.map(s=>{const n=s.sortable!==!1,i=e(s.key),l=s.width?`width: ${s.width};`:"",o=n?`data-sort-column="${s.key}"`:"";return`<th style="${l}${n?"cursor: pointer;":""}" ${o}>${s.label}${i}</th>`}).join(""),a=this.config.data.map(s=>`<tr>${this.config.columns.map(i=>{const l=s[i.key];return`<td>${i.render?i.render(l,s):l??"-"}</td>`}).join("")}</tr>`).join("");return`
