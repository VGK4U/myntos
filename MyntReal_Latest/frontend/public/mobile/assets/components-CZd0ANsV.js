...
import{t as x,j as I,d as v,b as S,n as C,r as w}from"./services-B7Il7GKb.js";import{L as E}from"./vendor-B1GRRwIS.js";import{_ as P}from"./vendor-capacitor-plugins-DTPsYLMi.js";import{gpsService as O}from"./services-gps-B4-ZgSBp.js";class V{modalEl=null;uiState="CLOSED";currentOptions=null;enteredNumber="";rawNumber="";isPresetNumber=!1;isDtmfOpen=!1;unsubscribeTelephony=null;currentSession=null;floatingPos={x:0,y:0};pillPos={x:0,y:0};isDraggingWindow=!1;isDraggingPill=!1;dragStartPointer={x:0,y:0};dragStartPos={x:0,y:0};hasInitializedPosition=!1;lastNotifiedLeadId=null;constructor(){typeof window<"u"&&(window.addEventListener("keydown",e=>{e.key==="Escape"&&this.isOpen()&&(this.uiState==="ACTIVE_FLOATING"?this.minimize():this.uiState==="DIALER"&&this.close())}),window.addEventListener("resize",()=>{this.isOpen()&&(this.clampPositions(),this.applyPositions())}))}isOpen(){return this.modalEl!==null&&document.body.contains(this.modalEl)&&this.uiState!=="CLOSED"}getUIState(){return this.uiState}open(e){this.currentOptions=e;const t=(e.phoneNumber||"").replace(/[^\d+]/g,"");this.enteredNumber=t,this.rawNumber=t,this.isPresetNumber=!!t,this.isDtmfOpen=!1,this.hasInitializedPosition=!1,this.unsubscribeTelephony&&this.unsubscribeTelephony(),this.unsubscribeTelephony=x.subscribe(n=>{this.currentSession=n,this.handleTelephonyStateUpdate(n)});const a=x.isCallActive();this.uiState=a?"ACTIVE_FLOATING":"DIALER",this.render(),this.loadLeadContext(e.entityId),e.autoStart&&this.enteredNumber&&this.startCall()}minimize(){this.uiState!=="CLOSED"&&(this.uiState="MINIMIZED",this.updateVisibility())}restore(){this.uiState!=="CLOSED"&&(this.uiState="ACTIVE_FLOATING",this.updateVisibility(),this.updateSessionUI())}close(){if(this.currentSession&&(x.isCallActive()||["connecting","ringing","connected","held"].includes(this.currentSession.state))){this.minimize();return}this.teardown()}forceCloseAndHangup(){this.currentSession&&x.isCallActive()&&x.endCall(),this.teardown()}teardown(){this.unsubscribeTelephony&&(this.unsubscribeTelephony(),this.unsubscribeTelephony=null),this.modalEl&&(this.modalEl.remove(),this.modalEl=null),this.uiState="CLOSED",this.currentOptions=null,this.currentSession=null,this.hasInitializedPosition=!1,this.lastNotifiedLeadId=null}handleTelephonyStateUpdate(e){if(!this.modalEl||!document.body.contains(this.modalEl))if(e.state!=="idle")this.render();else return;if(x.isCallActive()){this.uiState==="DIALER"&&(this.uiState="ACTIVE_FLOATING");const a=Number(this.currentOptions?.entityId);a&&(this.currentOptions?.entityType==="lead"||!this.currentOptions?.entityType)&&this.lastNotifiedLeadId!==a&&(this.lastNotifiedLeadId=a,I.notifyCallActive(a))}else if(this.lastNotifiedLeadId=null,e.state==="ended"){this.uiState="ENDED_SUMMARY";const a=this.currentOptions,n=e.durationSeconds||0;I.clearCallActive(),a?.entityId&&window.dispatchEvent(new CustomEvent("myntos:lead-call-ended",{detail:{leadId:a.entityId,name:a.name,phoneNumber:a.phoneNumber,durationSeconds:n,categoryName:a.categoryName,status:a.status}})),setTimeout(()=>{this.uiState==="ENDED_SUMMARY"&&this.teardown()},1600)}else if(e.state==="idle"&&this.uiState!=="DIALER"){I.clearCallActive(),this.teardown();return}this.updateVisibility(),this.updateSessionUI()}maskPhone(e){if(!e)return"—";const t=e.replace(/\D/g,"");if(t.length<6)return e;const a=t.slice(-10);return v.isMR10001()?a.length===10?`+91 ${a.slice(0,5)} ${a.slice(5)}`:e:`+91 ${a.slice(0,2)}••••${a.slice(-4)}`}getDialInputDisplay(){return!v.isMR10001()&&this.isPresetNumber&&this.enteredNumber?this.maskPhone(this.enteredNumber):this.enteredNumber}formatDuration(e){const t=Math.floor(e/60).toString().padStart(2,"0"),a=(e%60).toString().padStart(2,"0");return`${t}:${a}`}initPositions(){if(this.hasInitializedPosition||typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),n=440,i=Math.max(12,Math.round((e-a)/2)),s=Math.max(20,Math.round((t-n)/2));this.floatingPos={x:i,y:s},this.pillPos={x:Math.max(12,e-260),y:Math.max(20,t-100)},this.hasInitializedPosition=!0}clampPositions(){if(typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),n=440,i=12,s=Math.max(12,e-a-12),c=12,o=Math.max(12,t-n-12);this.floatingPos.x=Math.min(Math.max(this.floatingPos.x,i),s),this.floatingPos.y=Math.min(Math.max(this.floatingPos.y,c),o);const r=240,d=54,u=12,l=Math.max(12,e-r-12),f=12,m=Math.max(12,t-d-12);this.pillPos.x=Math.min(Math.max(this.pillPos.x,u),l),this.pillPos.y=Math.min(Math.max(this.pillPos.y,f),m)}applyPositions(){const e=this.modalEl?.querySelector("#spModalDialog");e&&(this.uiState==="ACTIVE_FLOATING"||this.uiState==="DIALER")&&(e.style.transform=`translate3d(${this.floatingPos.x}px, ${this.floatingPos.y}px, 0)`);const t=this.modalEl?.querySelector("#spMinimizedPill");t&&this.uiState==="MINIMIZED"&&(t.style.transform=`translate3d(${this.pillPos.x}px, ${this.pillPos.y}px, 0)`)}render(){this.modalEl&&this.modalEl.remove(),this.initPositions(),this.clampPositions();const{name:e,entityType:t}=this.currentOptions||{},a=e||"Customer Lead",n=t?t.toUpperCase():"LEAD";this.modalEl=document.createElement("div"),this.modalEl.id="myntosCentralSoftphoneModal",this.modalEl.style.cssText=`
    ...
    `}attachEventListeners(){this.container&&(document.getElementById("drawerClose")?.addEventListener("click",()=>this.close()),this.container.querySelectorAll("[data-toggle]").forEach(e=>{e.addEventListener("click",t=>{const a=e.dataset.toggle;this.toggleSection(a),t.stopPropagation()})}),this.container.querySelectorAll("[data-route]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.route,a=e.dataset.tab;a?w.navigate(t,{tab:a}):w.navigate(t),this.close()})}),document.getElementById("drawerLogout")?.addEventListener("click",async()=>{this.close(),confirm("Are you sure you want to logout?")&&await v.logout()}))}toggleSection(e){this.expandedSections.has(e)?this.expandedSections.delete(e):this.expandedSections.add(e),this.updateUI()}updateUI(){this.container&&(this.container.innerHTML=this.render(),this.attachEventListeners())}async loadStaffMenus(){try{const e=await S.get("/staff/menu-settings/my-menus?unified=true");if(e.success&&e.data&&e.data.sidebar_tree){this.staffMenuTree=e.data.sidebar_tree,this.isStaffMenuLoaded=!0;try{localStorage.setItem("mnr_staff_menu_tree_cache",JSON.stringify(this.staffMenuTree))}catch{}this.updateUI()}}catch(e){console.error("Failed to load dynamic staff menus:",e)}}filterMenusForRole(e,t,a=!1){const i=v.getAuthState().user||{},s=(i.emp_code||i.employee_code||"").toString().toUpperCase().trim(),c=["MN10009","MR10022","MR10036","MR10027","MN10017","MN10016"].includes(s);if(t&&!c)return e;const o=new Set(["SAAS","SAAS_MANAGEMENT","SAAS CONFIGURATION","CONFIGURATION","SYSTEM_CONFIGURATION","SYSTEM CONFIG","META_ADS","META ADS","VENDOR_MANAGEMENT","VENDOR MANAGEMENT","VENDORS","HR","HR_MANAGEMENT","ZYNOVA","ZYNOVA_REAL_ESTATE","MNR","MNR_USER_SIDEBAR","MNR USER SIDEBAR","MNR_USER","MNR USER"]);a||(o.add("ACCOUNTS"),o.add("ACCOUNTS_EARNINGS"),o.add("ACCOUNTS & EARNINGS"),o.add("FINANCE"),o.add("FINANCE & EARNINGS"),o.add("FINANCE_EARNINGS"),o.add("ACCOUNTS_MANAGEMENT"));const r=new Set(["EXECUTIVE_DASHBOARD","CATEGORY_LEADS_MASTER","SAAS_CONFIG","SYSTEM_CONFIG"]);(!t||c)&&(r.add("LEADS_MASTER"),r.add("STAFF_LEADS"),r.add("staff_leads")),a||(r.add("PAYROLL_PROFILE"),r.add("SALARY_SLIPS"),r.add("EXPENSE_ENTRIES"));const d=[];for(const u of e){const l=(u.section_code||"").toUpperCase().trim(),f=(u.section_label||"").toUpperCase().trim();if(o.has(l)||o.has(f))continue;let m=u.items;m&&m.length>0&&(m=m.filter(y=>{const k=(y.menu_code||"").toUpperCase().trim();return!r.has(k)}));let g=u.subSections;g&&g.length>0&&(g=g.map(y=>({...y,items:y.items.filter(k=>{const _=(k.menu_code||"").toUpperCase().trim();return!r.has(_)})})).filter(y=>y.items.length>0));const h=m&&m.length>0,b=g&&g.length>0;(h||b)&&d.push({...u,items:h?m:void 0,subSections:b?g:void 0})}return d}getStaffMenuMaster(){if(!this.staffMenuTree)return D;const e={EXECUTIVE_DASHBOARD:1,staff_executive_dashboard:1,mnr_executive_dashboard:1,MNR_BANK_WISE_LEADS:2,BANK_WISE_LEADS:2,staff_bank_wise_leads:2,CATEGORY_LEADS_MASTER:3,mnr_leads_master:3,staff_mnr_leads_master:3,SOLAR_LEADS:4,staff_solar_leads:4,mnr_solar_leads:4,EV_B2B_LEADS:5,staff_ev_b2b_leads:5,mnr_ev_b2b_leads:5,EV_B2C_LEADS:6,staff_ev_b2c_leads:6,mnr_ev_b2c_leads:6,EV_SPARES_LEADS:7,staff_ev_spares_leads:7,mnr_ev_spares_leads:7,ZYN_REAL_ESTATE:8,staff_real_dreams_leads:8,mnr_real_dreams_leads:8,ZYN_INSURANCE:9,staff_insurance_leads:9,mnr_insurance_leads:9,ETC_LEADS:10,staff_etc_leads:10,mnr_etc_leads:10,MNR_LEADS:11,staff_mnr_leads:11,mnr_category_leads:11,AUTO_DIALER:12,staff_auto_dialer:12,"auto-dialer":12},t={MY_CRM_DASHBOARD:1,staff_crm_dashboard:1,"staff-crm":1,MY_LEADS:2,staff_my_leads:2,"staff-my-leads":2,AUTO_DIALER:3,staff_auto_dialer:3,staff_dialer:3,"auto-dialer":3,SOFTPHONE:4,staff_softphone:4,softphone:4,CALLING_PAGE:4,calling:4,"phone-dialpad":4,BANK_WISE_LEADS:5,staff_bank_wise_leads:5,"staff-bank-wise-leads":5,TEAM_LEADS:6,staff_team_leads:6,"staff-team-leads":6,WHATSAPP_CENTER:7,staff_crm_whatsapp_inbox:7,"staff-whatsapp":7,CALL_TRACKING:8,call_tracking_dashboard:8,"staff-call-tracking":8,OPERATOR_CALLS:9,staff_operator_calls:9,"operator-calls":9,VGK_TEAM_MEMBERS:10,staff_vgk_members:10,"staff-vgk-members":10,STAFF_LEADS:11,staff_leads:11,LEADS_MASTER:11,leads_master:11},a=new Map,n=[],i=(o,r,d,u)=>{const l=N[d]||N[d?.replace(/\/$/,"")]||(d?d.replace(/^\/staff\//,"").replace(/\//g,"-"):null);if(!l)return null;const m=v.getAuthState().user||{},g=(m.emp_code||m.employee_code||"").toString().toUpperCase().trim();if(["MN10009","MR10022","MR10036","MR10027","MN10017","MN10016"].includes(g)&&(l==="staff-leads"||d&&d.toLowerCase()==="/staff/leads"||o&&["staff_leads","STAFF_LEADS","LEADS_MASTER","staff_leads_master"].includes(o)))return null;let h=r;(!h||h==="None"||h.trim()==="")&&(h=(o||"").replace(/^staff_|^_staff_|^mnr_/i,"").replace(/_/g," ").replace(/\b\w/g,L=>L.toUpperCase()));const b=(o||"").toUpperCase(),y=(d||"").toLowerCase();b.includes("AUTO_DIALER")||y.includes("auto-dialer")||y==="/staff/dialer"||b==="STAFF_AUTO_DIALER"||b==="AUTO_DIALER"||b==="STAFF_DIALER"||b==="DIALER"?h="Auto Dialer":b.includes("BANK_WISE_LEADS")||y.includes("bank-wise-leads")?h="Field staff leads":b.includes("REAL_DREAMS")||y.includes("real-dreams-leads")?h="Real Dreams Leads":b.includes("ETC_LEADS")||y.includes("etc-leads")?h="ETC Training Students":b.includes("CATEGORY_LEADS_MASTER")||b.includes("MNR_LEADS_MASTER")||y.includes("mnr-leads-master")?h="Category Lead Master":b.includes("WHATSAPP")||y.includes("whatsapp")?h="WhatsApp Center":b.includes("SOFTPHONE")||b==="PHONE_DIALPAD"||b==="CALLING_PAGE"||y.includes("softphone")||y.includes("calling")||y.includes("phone-dialpad")?h="Calling & Softphone":(b.includes("VGK_TEAM_MEMBERS")||b==="STAFF_VGK_MEMBERS"||y.includes("vgk/members"))&&(h="VGK Channel Partners");const k=u||(h.includes("WhatsApp")?"fab fa-whatsapp":h.includes("Auto Dialer")?"fas fa-phone-volume":h.includes("Softphone")||h.includes("Calling")?"fas fa-headset":h.includes("VGK")||h.includes("Channel")?"fas fa-users":h.includes("Field")?"fas fa-users-gear":"fas fa-file-alt"),_=h.includes("WhatsApp")?"color: #25d366;":h.includes("Auto Dialer")||h.includes("Softphone")||h.includes("Calling")?"color: #38bdf8;":h.includes("VGK")||h.includes("Channel")?"color: #7c3aed;":"",A=`<i class="${k}" style="margin-right: 8px; width: 18px; text-align: center; ${_}"></i>`;return{menu_code:o,label:`${A}${h}`,route:l}};for(const o of this.staffMenuTree){if(o.id==="progress"||o.section_id==="progress"||(o.title||"").toUpperCase()==="PROGRESS")continue;const r=[],d=[];if(o.items)for(const u of o.items){const l=i(u.menu_code,u.menu_name||u.label||u.name||u.title,u.route_path,u.menu_icon);l&&r.push(l)}if(o.subSections)for(const u of o.subSections){const l=[];if(u.items)for(const m of u.items){const g=i(m.menu_code,m.menu_name||m.label||m.name||m.title,m.route_path,m.menu_icon);g&&l.push(g)}let f=u.title||u.name||u.id||"Subsection";(f==="None"||!f)&&(f=(u.id||"").replace(/^staff_|^vm_/i,"").replace(/_/g," ").replace(/\b\w/g,m=>m.toUpperCase())),l.length>0&&d.push({sub_section_code:u.id||u.section_id||"sub",sub_section_label:f,items:l})}if(r.length>0||d.length>0){let u=(o.id||o.section_id||"other").toString().trim(),l=(o.title||o.name||"Other").toString().trim();(l==="None"||!l)&&(l=u.replace(/_/g," ").replace(/\b\w/g,g=>g.toUpperCase()));const f=u.toLowerCase(),m=l.toUpperCase();if((f==="mynt_real"||f==="myntreal"||f==="workflows"||m==="MYNT REAL"||m==="MYNTREAL"||m==="WORK FLOWS"||m==="WORKFLOWS")&&(u="WORKFLOWS",l="WORK FLOWS"),(f==="crm"||f==="crm_module"||f==="crm_leads"||f==="crm & leads"||m.includes("CRM"))&&(u="CRM_MODULE",l="CRM & LEADS"),a.has(u)){const g=a.get(u);if(r.length>0){g.items=g.items||[];const h=new Set(g.items.map(b=>b.menu_code));for(const b of r)h.has(b.menu_code)||(g.items.push(b),h.add(b.menu_code))}d.length>0&&(g.subSections=g.subSections||[],g.subSections.push(...d))}else n.push(u),a.set(u,{section_code:u,section_label:l,order:u==="CRM_MODULE"?4:o.order!==void 0?o.order:999,items:r.length>0?r:void 0,subSections:d.length>0?d:void 0})}}let s=a.get("CRM_MODULE")||a.get("CRM_LEADS")||a.get("crm")||a.get("CRM");if(s||(s={section_code:"CRM_MODULE",section_label:"CRM & LEADS",order:4,items:[]},a.set("CRM_MODULE",s),n.push("CRM_MODULE")),s){if(s.items=s.items||[],!s.items.some(d=>d.route==="auto-dialer"||d.menu_code&&(d.menu_code.toUpperCase().includes("AUTO_DIALER")||d.menu_code.toUpperCase().includes("DIALER"))))s.items.push({menu_code:"AUTO_DIALER",label:'<i class="fas fa-phone-volume" style="margin-right: 8px; width: 18px; text-align: center; color: #38bdf8;"></i>Auto Dialer',route:"auto-dialer"});else for(const d of s.items)(d.route==="auto-dialer"||d.menu_code&&(d.menu_code.toUpperCase().includes("AUTO_DIALER")||d.menu_code.toUpperCase().includes("DIALER")))&&(d.label='<i class="fas fa-phone-volume" style="margin-right: 8px; width: 18px; text-align: center; color: #38bdf8;"></i>Auto Dialer');s.items.some(d=>d.route==="softphone"||d.menu_code&&(d.menu_code.toUpperCase().includes("SOFTPHONE")||d.menu_code.toUpperCase().includes("CALLING")))||s.items.push({menu_code:"SOFTPHONE",label:'<i class="fas fa-headset" style="margin-right: 8px; width: 18px; text-align: center; color: #38bdf8;"></i>Calling & Softphone',route:"softphone"})}const c=n.map(o=>a.get(o));c.sort((o,r)=>o.order-r.order);for(const o of c)o.section_code==="WORKFLOWS"&&o.items&&o.items.sort((r,d)=>(e[r.menu_code]||99)-(e[d.menu_code]||99)),o.section_code==="CRM_MODULE"&&o.items&&o.items.sort((r,d)=>(t[r.menu_code]||t[r.route]||99)-(t[d.menu_code]||t[d.route]||99));return c}open(){if(this.isOpen)return;C.getPortal()==="staff"&&!this.isStaffMenuLoaded&&this.loadStaffMenus(),this.updateUI(),this.isOpen=!0,this.container?.classList.add("open"),this.overlay?.classList.add("visible"),document.body.style.overflow="hidden"}close(){this.isOpen&&(this.isOpen=!1,this.container?.classList.remove("open"),this.overlay?.classList.remove("visible"),document.body.style.overflow="")}toggle(){this.isOpen?this.close():this.open()}}let T=null;function F(){return T||(T=new G),T}class B{static render(e){let{title:t,showBack:a=!1,showLogout:n=!1,rightAction:i,subtitle:s,showMenu:c}=e;const o=C.getPortal(),r=w.getCurrentRoute(),d=["progress","dashboard","attendance","journeys","announcements","profile","mnr-dashboard","partner-dashboard","vgk-member-hub"].includes(r);if(c===void 0&&(c=d||!a),o==="vgk"&&(n=!0,!s)){const l=v.getAuthState().user||{},f=l.name||l.partner_name||"",m=l.partner_code||"";(f||m)&&(s=m?`${f} (${m})`:f)}return`
    ...
    `}static getPortalDashboard(){const e=C.getPortal();return e==="mnr"?"mnr-dashboard":e==="partner"?"partner-dashboard":e==="vgk"?"vgk-member-hub":"progress"}static attachListeners(e){let{showMenu:t,showBack:a=!1,rightAction:n,showLogout:i=!1}=e;t===void 0&&(t=!!document.getElementById("hamburgerBtn")),C.getPortal()==="vgk"&&(i=!0),t&&document.getElementById("hamburgerBtn")?.addEventListener("click",()=>{F().open()}),a&&document.getElementById("backBtn")?.addEventListener("click",()=>{w.goBack()||w.navigate(B.getPortalDashboard())}),n&&document.getElementById("headerActionBtn")?.addEventListener("click",n.onClick),i&&document.getElementById("logoutBtn")?.addEventListener("click",async()=>{confirm("Are you sure you want to logout?")&&await v.logout()})}static attachBackHandler(){document.getElementById("backBtn")?.addEventListener("click",()=>{w.goBack()||w.navigate(B.getPortalDashboard())})}}class re{constructor(e){this.options=e,this.trackPoints=e.trackPoints,this.stops=e.stops||[],this.onViewChange=e.onViewChange}options;map=null;container=null;trackPoints=[];stops=[];routeLine=null;progressLine=null;currentMarker=null;startMarker=null;endMarker=null;stopMarkers=[];currentView="street";tileLayers={};playbackIndex=0;isPlaying=!1;playbackInterval=null;playbackSpeed=2;onViewChange;addressCache=new Map;mount(){if(this.container=document.getElementById(this.options.containerId),!this.container){console.error("[LeafletMap] Container not found:",this.options.containerId);return}this.render(),this.initMap()}render(){this.container&&(this.container.innerHTML=`
...
இ-காமர்ஸ் மூலம் பார்வையிட: http://localhost:5001/ecom?segment=real-dreams`}},ev_b2c:{name:"Smart Electric 2-Wheelers (B2C)",btnLabel:"EV 2W",segmentSlug:"ev-b2c",catalogSlug:"ev-smart-commuter",desc:"Sends Smart Electric 2W catalog with 120km range, LFP battery, mobile app & ₹0.25/km running cost.",messages:{te:(p,e)=>`నమస్కారం ${p} గారు! 🙏
...
పై లింక్ క్లిక్ చేసి లేటెస్ట్ ఈవీ మోడల్స్, ఫీచర్లు మరియు టెస్ట్ రైడ్ బుక్ చేసుకోండి.`,en:(p,e)=>`Namaskaram ${p}! 🙏
...
Click the link above to explore EV models, color variants & book your free test ride.`,hi:(p,e)=>`नमस्ते ${p} जी! 🙏
...
कृपया ऊपर दिए गए लिंक पर क्लिक करें और मॉडल्स देखें व फ्री टेस्ट राइड बुक करें।`,ta:(p,e)=>`வணக்கம் ${p}! 🙏
...
👉 ${s}`}applyDigitalCatalog(e){e&&(this.selectedCatalogLang=e,this.modalEl?.querySelectorAll(".uwa-cat-lang-btn")?.forEach(c=>{const o=c.getAttribute("data-lang")===e;c.style.background=o?"#16a34a":"#fff",c.style.color=o?"#fff":"#334155",c.style.borderColor=o?"#16a34a":"#cbd5e1",c.style.fontWeight=o?"700":"600"}));const t=document.getElementById("uwaDigitalCatSel");t&&t.value&&(this.selectedCatalogKey=t.value);const a=this.getDigitalCatalogMessage(this.selectedCatalogKey,this.selectedCatalogLang),n=this.getSenderSignature(),i=document.getElementById("uwaMessageText");i&&(i.value=a+n,i.focus())}detectCatalogKey(){const e=(this.currentOptions?.context||"").toLowerCase(),t=(this.currentOptions?.segment||"").toLowerCase();return e.includes("solar")||t==="solar"?"solar":e.includes("real")||e.includes("property")||e.includes("estate")||t==="myntreal_real"?"real_estate":e.includes("spare")||t==="ev_spares"?"ev_spares":e.includes("cargo")||e.includes("fleet")||e.includes("b2b")||t==="ev_b2b"?"ev_b2b":e.includes("ev")||e.includes("zynova")||e.includes("vehicle")||t==="ev_b2c"?"ev_b2c":e.includes("train")||e.includes("skill")||e.includes("etc")||t==="etc_training"?"etc_training":e.includes("insur")||e.includes("policy")||e.includes("care")?"insurance":e.includes("pricing")||e.includes("margin")||e.includes("commercial")||t==="hub_pricing"?"hub_pricing":e.includes("hub")||e.includes("franchise")?"industrial_hub":"solar"}async loadCanonicalTemplates(e){const t=document.getElementById("uwaCanonicalTpl"),a=document.getElementById("uwaNoTplNotice");if(!t)return;t.innerHTML='<option value="">— Loading templates… —</option>',a&&(a.style.display="none");const n=this.activeMode==="scanned"?"scanned":"company";let i=`/whatsapp-config/templates?mode=${encodeURIComponent(n)}`;try{const s=await S.get(i),c=s?.templates||s?.data||s||[];this.allCanonicalTemplates=Array.isArray(c)?c:[],this.filterCanonicalTemplates(e)}catch{t.innerHTML='<option value="">— Error loading templates —</option>'}}filterCanonicalTemplates(e){const t=document.getElementById("uwaCanonicalTpl"),a=document.getElementById("uwaNoTplNotice");if(!t)return;const n=document.getElementById("uwaCanonicalSeg"),i=document.getElementById("uwaCanonicalCat"),s=document.getElementById("uwaCanonicalSearch"),c=(n?.value||"").toLowerCase().trim(),o=(i?.value||"").toUpperCase().trim(),r=(s?.value||"").toLowerCase().trim();let d=this.allCanonicalTemplates||[];if(c&&(d=d.filter(l=>(l.segment||"").toLowerCase()===c)),o&&(d=d.filter(l=>(l.category||"").toUpperCase()===o)),r&&(d=d.filter(l=>{const f=(l.template_name||l.name||"").toLowerCase(),m=(l.body_text||l.content||l.body||"").toLowerCase(),g=(l.slug||"").toLowerCase();return f.includes(r)||m.includes(r)||g.includes(r)})),this.canonicalTemplates=d,!d.length){t.innerHTML=`
      ...
      `,a&&(a.style.display="block"),this.onCanonicalTplChange();return}a&&(a.style.display="none");let u=`<option value="">— Select template (${d.length} available) —</option>`;u+='<option value="__create_new__" style="color:#16a34a; font-weight:700;">➕ Create / Add Template for this Segment...</option>',d.forEach(l=>{const f=l.segment?`[${l.segment}] `:"";u+=`<option value="${l.id}">${f}${this.escapeHtml(l.template_name||l.name||"Template #"+l.id)} (${l.category||"MARKETING"})</option>`}),t.innerHTML=u,e&&(t.value=String(e),this.onCanonicalTplChange())}toggleAddTemplateCard(e){const t=document.getElementById("uwaAddTplCard");if(!t)return;const a=t.style.display!=="none",n=e!==void 0?e:!a;if(t.style.display=n?"block":"none",n){const i=document.getElementById("uwaCanonicalSeg")?.value||this.detectSegment(),s=document.getElementById("uwaNewTplSeg");s&&i&&(s.value=i),document.getElementById("uwaNewTplName")?.focus()}}async saveNewTemplate(){const e=document.getElementById("uwaNewTplName"),t=document.getElementById("uwaNewTplSeg"),a=document.getElementById("uwaNewTplBody"),n=document.getElementById("uwaSaveNewTplBtn"),i=(e?.value||"").trim(),s=t?.value||"general",c=(a?.value||"").trim();if(!i){this.showFeedback("Please enter a template name.","error");return}if(!c){this.showFeedback("Please enter the template message body.","error");return}n&&(n.disabled=!0);const o=i.toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"").slice(0,40);try{const r=await S.post("/whatsapp-config/templates",{name:i,slug:o,category:"MARKETING",segment:s,body_text:c,language:"en",usage_scope:"all"});if(r?.success){this.showFeedback(`✅ Template "${i}" created & approved for WhatsApp API!`,"success"),this.toggleAddTemplateCard(!1),e&&(e.value=""),a&&(a.value="");const d=document.getElementById("uwaCanonicalSeg");d&&(d.value=s);const u=r.template?.id||r.id||o;await this.loadCanonicalTemplates(u)}else this.showFeedback(r?.error||"Failed to create template","error")}catch(r){this.showFeedback(`Error creating template: ${r.message||"Server error"}`,"error")}finally{n&&(n.disabled=!1)}}detectSegment(){if(this.currentOptions?.segment)return this.currentOptions.segment;const e=(this.currentOptions?.context||"").toLowerCase();return e.includes("solar")?"solar":e.includes("real")||e.includes("property")||e.includes("estate")?"myntreal_real":e.includes("spare")?"EV_SPARES":e.includes("ev")||e.includes("vehicle")||e.includes("zynova")?"ev_b2c":e.includes("train")||e.includes("skill")||e.includes("etc")?"etc_training":e.includes("partner")?"partner":e.includes("staff")?"staff":e.includes("vgk")?"vgk":"general"}onCanonicalTplChange(){const e=document.getElementById("uwaCanonicalTpl"),t=document.getElementById("uwaCanonicalVarsWrap"),a=document.getElementById("uwaCanonicalVarsBox"),n=e?.value;if(n==="__create_new__"){this.toggleAddTemplateCard(!0),e.value="";return}if(!n){t&&(t.style.display="none"),a&&(a.innerHTML="");return}const i=this.canonicalTemplates.find(o=>String(o.id)===String(n)||String(o.slug)===String(n));if(!i)return;this.selectedCanonicalBody=i.body_text||i.content||i.body||"";const s=this.selectedCanonicalBody.match(/\{\{(\d+)\}\}/g)||[],c=[];if(s.forEach(o=>{const r=o.replace(/[\{\}]/g,"");c.includes(r)||c.push(r)}),c.sort((o,r)=>Number(o)-Number(r)),c.length&&a&&t){t.style.display="block";let o="";c.forEach(r=>{const d=r==="1"&&this.currentOptions?.name||"";o+=`
