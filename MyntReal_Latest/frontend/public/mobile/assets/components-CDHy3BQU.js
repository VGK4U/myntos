const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./services-BJMIjrrK.js","./vendor-capacitor-plugins-DTPsYLMi.js","./vendor-capacitor-core-zwN_Y1fq.js","./vendor-B1GRRwIS.js"])))=>i.map(i=>d[i]);
import{t as g,n as S,d as b,r as E,b as _}from"./services-BJMIjrrK.js";import{L as v}from"./vendor-B1GRRwIS.js";import{_ as I}from"./vendor-capacitor-plugins-DTPsYLMi.js";import{gpsService as N}from"./services-gps-Dm_lZvrs.js";class B{modalEl=null;uiState="CLOSED";currentOptions=null;enteredNumber="";isDtmfOpen=!1;unsubscribeTelephony=null;currentSession=null;floatingPos={x:0,y:0};pillPos={x:0,y:0};isDraggingWindow=!1;isDraggingPill=!1;dragStartPointer={x:0,y:0};dragStartPos={x:0,y:0};hasInitializedPosition=!1;constructor(){typeof window<"u"&&(window.addEventListener("keydown",e=>{e.key==="Escape"&&this.isOpen()&&(this.uiState==="ACTIVE_FLOATING"?this.minimize():this.uiState==="DIALER"&&this.close())}),window.addEventListener("resize",()=>{this.isOpen()&&(this.clampPositions(),this.applyPositions())}))}isOpen(){return this.modalEl!==null&&document.body.contains(this.modalEl)&&this.uiState!=="CLOSED"}getUIState(){return this.uiState}open(e){this.currentOptions=e,this.enteredNumber=(e.phoneNumber||"").replace(/[^\d+]/g,""),this.isDtmfOpen=!1,this.hasInitializedPosition=!1,this.unsubscribeTelephony&&this.unsubscribeTelephony(),this.unsubscribeTelephony=g.subscribe(a=>{this.currentSession=a,this.handleTelephonyStateUpdate(a)});const t=g.isCallActive();this.uiState=t?"ACTIVE_FLOATING":"DIALER",this.render(),e.autoStart&&this.enteredNumber&&this.startCall()}minimize(){this.uiState!=="CLOSED"&&(this.uiState="MINIMIZED",this.updateVisibility())}restore(){this.uiState!=="CLOSED"&&(this.uiState="ACTIVE_FLOATING",this.updateVisibility(),this.updateSessionUI())}close(){if(this.currentSession&&g.isCallActive()){this.minimize();return}this.teardown()}forceCloseAndHangup(){this.currentSession&&g.isCallActive()&&g.endCall(),this.teardown()}teardown(){this.unsubscribeTelephony&&(this.unsubscribeTelephony(),this.unsubscribeTelephony=null),this.modalEl&&(this.modalEl.remove(),this.modalEl=null),this.uiState="CLOSED",this.currentOptions=null,this.currentSession=null,this.hasInitializedPosition=!1}handleTelephonyStateUpdate(e){if(!this.modalEl||!document.body.contains(this.modalEl))if(e.state!=="idle")this.render();else return;if(g.isCallActive())this.uiState==="DIALER"&&(this.uiState="ACTIVE_FLOATING");else if(e.state==="ended")this.uiState="ENDED_SUMMARY",setTimeout(()=>{this.uiState==="ENDED_SUMMARY"&&this.teardown()},1600);else if(e.state==="idle"&&this.uiState!=="DIALER"){this.teardown();return}this.updateVisibility(),this.updateSessionUI()}maskPhone(e){if(!e)return"—";const t=e.replace(/\D/g,"");if(t.length<6)return e;const a=t.slice(-10);return`+91 ${a.slice(0,2)}••••${a.slice(-4)}`}formatDuration(e){const t=Math.floor(e/60).toString().padStart(2,"0"),a=(e%60).toString().padStart(2,"0");return`${t}:${a}`}initPositions(){if(this.hasInitializedPosition||typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),n=440,i=Math.max(12,Math.round((e-a)/2)),s=Math.max(20,Math.round((t-n)/2));this.floatingPos={x:i,y:s},this.pillPos={x:Math.max(12,e-260),y:Math.max(20,t-100)},this.hasInitializedPosition=!0}clampPositions(){if(typeof window>"u")return;const e=window.innerWidth,t=window.innerHeight,a=Math.min(400,e-24),n=440,i=12,s=Math.max(12,e-a-12),o=12,l=Math.max(12,t-n-12);this.floatingPos.x=Math.min(Math.max(this.floatingPos.x,i),s),this.floatingPos.y=Math.min(Math.max(this.floatingPos.y,o),l);const r=240,d=54,p=12,m=Math.max(12,e-r-12),h=12,c=Math.max(12,t-d-12);this.pillPos.x=Math.min(Math.max(this.pillPos.x,p),m),this.pillPos.y=Math.min(Math.max(this.pillPos.y,h),c)}applyPositions(){const e=this.modalEl?.querySelector("#spModalDialog");e&&(this.uiState==="ACTIVE_FLOATING"||this.uiState==="DIALER")&&(e.style.transform=`translate3d(${this.floatingPos.x}px, ${this.floatingPos.y}px, 0)`);const t=this.modalEl?.querySelector("#spMinimizedPill");t&&this.uiState==="MINIMIZED"&&(t.style.transform=`translate3d(${this.pillPos.x}px, ${this.pillPos.y}px, 0)`)}render(){this.modalEl&&this.modalEl.remove(),this.initPositions(),this.clampPositions();const{name:e,entityType:t}=this.currentOptions||{},a=e||"Customer Lead",n=t?t.toUpperCase():"LEAD";this.modalEl=document.createElement("div"),this.modalEl.id="myntosCentralSoftphoneModal",this.modalEl.style.cssText=`
    ...
    `}renderSection(e){const t=this.expandedSections.has(e.section_code),a=e.subSections&&e.subSections.length>0,i=e.items&&e.items.length>0;return a?`
        <div class="drawer-section" data-section="${e.section_code}">
          ...
          <div class="section-items ${t?"expanded":""}" style="display: ${t?"block":"none"};">
            ${e.subSections.map(n=>this.renderSubSection(n)).join("")}
          </div>
        </div>
      `:i?`
        <div class="drawer-section" data-section="${e.section_code}">
    ...
    `,document.head.appendChild(e)}setPlaybackIndex(e){e<0||e>=this.trackPoints.length||(this.playbackIndex=e,this.updatePlaybackUI())}getPlaybackState(){return{index:this.playbackIndex,total:this.trackPoints.length,isPlaying:this.isPlaying}}destroy(){this.stopPlayback(),this.map&&(this.map.remove(),this.map=null)}}const A={greeting:{label:"👋 Welcome & Introduction",text:"Namaskaram! Thank you for connecting with MyntReal. I am your dedicated relationship manager. Please let me know how I may assist you with your project today."},bank_update:{label:"🏦 Bank Loan Update",text:"Dear Customer, your bank file is currently under active processing. Our team is following up with the branch for swift approval and sanction."},net_meter:{label:"⚡ Net Meter & EB",text:"Dear Customer, your DISCOM Net Metering and EB service documentation is progressing as scheduled. We will update you once the inspection is cleared."},payment:{label:"💰 Payment / Balance Follow-up",text:"Dear Customer, this is a gentle reminder regarding the pending balance for your project. Kindly arrange the clearance at your earliest convenience."},site_visit:{label:"📍 Location & Site Visit",text:"Dear Customer, our technical field staff is scheduled to visit your site. Kindly let us know if you need to coordinate the visit time."}};class z{modalEl=null;currentOptions=null;activeMode="scanned";getSenderSignature(){const t=b.getAuthState().user||{};return`

Regards,
${t.full_name||t.name||`${t.first_name||""} ${t.last_name||""}`.trim()||"Staff"}`}open(e){this.currentOptions=e,this.activeMode="scanned",this.render()}close(){this.modalEl&&(this.modalEl.remove(),this.modalEl=null)}render(){if(this.close(),!this.currentOptions)return;const{phone:e,name:t,context:a,defaultMessage:n}=this.currentOptions,i=(e||"").replace(/\D/g,"").slice(-10),s=this.getSenderSignature(),o=(n||A.greeting.text)+s;this.modalEl=document.createElement("div"),this.modalEl.id="unifiedWAModal",this.modalEl.className="uwa-modal-backdrop",this.modalEl.innerHTML=`
        ...
        <div class="uwa-header">
          <div class="uwa-header-info">
            ...
            <div class="uwa-recipient-sub">
              <strong>${this.escapeHtml(t||"Customer")}</strong> · +91 ${n}
    ...
    `}attachEventListeners(){this.container&&(document.getElementById("mnrDrawerClose")?.addEventListener("click",()=>this.close()),this.container.querySelectorAll("[data-toggle]").forEach(e=>{e.addEventListener("click",t=>{const a=e.dataset.toggle;this.toggleSection(a),t.stopPropagation()})}),this.container.querySelectorAll("[data-route]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.route;E.navigate(t),this.close()})}),document.getElementById("mnrLogoutBtn")?.addEventListener("click",async()=>{confirm("Are you sure you want to logout?")&&(await b.logout(),window.dispatchEvent(new CustomEvent("logout")),this.close())}))}toggleSection(e){this.expandedSections.has(e)?this.expandedSections.delete(e):this.expandedSections.add(e),this.updateUI()}updateUI(){this.container&&(this.container.innerHTML=this.render(),this.attachEventListeners())}open(){this.isOpen||(this.isOpen=!0,this.container?.classList.add("open"),this.overlay?.classList.add("visible"),document.body.style.overflow="hidden")}close(){this.isOpen&&(this.isOpen=!1,this.container?.classList.remove("open"),this.overlay?.classList.remove("visible"),document.body.style.overflow="")}toggle(){this.isOpen?this.close():this.open()}destroy(){this.container?.remove(),this.overlay?.remove()}}const ie=new F;class se{config;constructor(e){this.config=e}render(){if(this.config.loading)return`
      ...
      `;const e=i=>this.config.sortColumn===i?this.config.sortDirection==="asc"?" ↑":" ↓":"",t=this.config.columns.map(i=>{const n=i.sortable!==!1,s=e(i.key),o=i.width?`width: ${i.width};`:"",l=n?`data-sort-column="${i.key}"`:"";return`<th style="${o}${n?"cursor: pointer;":""}" ${l}>${i.label}${s}</th>`}).join(""),a=this.config.data.map(i=>`<tr>${this.config.columns.map(s=>{const o=i[s.key];return`<td>${s.render?s.render(o,i):o??"-"}</td>`}).join("")}</tr>`).join("");return`
    ...
    `}attachEventListeners(){this.container&&(document.getElementById("partnerDrawerClose")?.addEventListener("click",()=>this.close()),this.container.querySelectorAll("[data-toggle]").forEach(e=>{e.addEventListener("click",t=>{const a=e.dataset.toggle;this.toggleSection(a),t.stopPropagation()})}),this.container.querySelectorAll("[data-route]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.route;E.navigate(t),this.close()})}),document.getElementById("partnerLogoutBtn")?.addEventListener("click",async()=>{confirm("Are you sure you want to logout?")&&(await b.logout(),window.dispatchEvent(new CustomEvent("logout")),this.close())}))}toggleSection(e){this.expandedSections.has(e)?this.expandedSections.delete(e):this.expandedSections.add(e),this.updateUI()}updateUI(){this.container&&(this.container.innerHTML=this.render(),this.attachEventListeners())}open(){this.isOpen||(this.isOpen=!0,this.container?.classList.add("open"),this.overlay?.classList.add("visible"),document.body.style.overflow="hidden")}close(){this.isOpen&&(this.isOpen=!1,this.container?.classList.remove("open"),this.overlay?.classList.remove("visible"),document.body.style.overflow="")}toggle(){this.isOpen?this.close():this.open()}destroy(){this.container?.remove(),this.overlay?.remove()}}const oe=new Y;class re{container;constructor(e){this.container=e}render(){const t=b.getAuthState().user?.portal||"staff",a=E.getTabRoutes(t),n=E.getCurrentRoute();this.container.innerHTML=`
