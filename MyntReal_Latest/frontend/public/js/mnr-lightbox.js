...
(function() {
    ...
    function createLightboxHTML() {
        const lightboxHTML = `
            <div id="mnrLightbox" class="mnr-lightbox" style="display: none;">
                ...
                <div class="mnr-lightbox-container">
                    ...
                    <button class="mnr-lightbox-download" aria-label="Download with contact details" title="Save with your contact details">
        ...
        const lightboxStyles = `
            <style id="mnrLightboxStyles">
                ...
                .mnr-lightbox-download::after {
                    content: ' Save';
                }

                .mnr-lightbox-download:hover {
                    background: rgba(109, 40, 217, 0.95);
                    transform: translateX(-50%) scale(1.05);
                    box-shadow: 0 4px 18px rgba(124, 58, 237, 0.5);
                }

                .mnr-lightbox-download.video-mode {
                    display: none;
                }
                ...
                .mnr-lightbox-dot:hover:not(.active) {
        ...
        // Safety check for document.head and document.body
        if (!document.head || !document.body) {
            console.warn('[MNR Lightbox] DOM not ready, deferring initialization');
    ...
    function initLightboxEvents() {
        ...
        // Download button — calls _vgkDlClick (contact overlay) if available, else direct download
        var dlBtn = lightbox.querySelector('.mnr-lightbox-download');
        if (dlBtn) {
            dlBtn.addEventListener('click', function() {
                var item = mediaItems[currentMediaIndex];
                if (!item || !item.url) return;
                var url  = item.url;
                var type = (item.type === 'video') ? 'video' : 'image';
                if (typeof window._vgkDlClick === 'function') {
                    window._vgkDlClick(url, url, type);
                } else {
    ...
    function showMedia(index) {
        ...
        if (!item.url) {
            ...
            const videoUrl = escapeAttr(item.url) + (escapeAttr(item.url).includes('#') ? '' : '#t=0.001');
            mediaHTML = `
        ...
        // Show/hide download button based on media type (hide for video; show for images)
        var dlBtn2 = document.querySelector('.mnr-lightbox-download');
        if (dlBtn2) {
            var isVideo = item && (item.type === 'video' || (item.file_type && item.file_type.startsWith('video/')));
            dlBtn2.style.display = isVideo ? 'none' : 'flex';
        }
        
        // Update counter — render highlighted navigation dots
        if (mediaItems.length > 1) {
            counterContainer.style.display = 'flex';
            counterContainer.innerHTML = mediaItems.map(function(_, i) {
                return '<button class="mnr-lightbox-dot' + (i === index ? ' active' : '') + '" aria-label="Go to image ' + (i + 1) + '" data-idx="' + i + '"></button>';
            }).join('');
            counterContainer.querySelectorAll('.mnr-lightbox-dot').forEach(function(dot) {
