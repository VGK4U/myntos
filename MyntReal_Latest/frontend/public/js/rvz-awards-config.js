...
function getUserId() {
    // For staff users, get emp_code from localStorage
    const staffUser = localStorage.getItem('staff_user');
    if (staffUser) {
        try {
            const userData = JSON.parse(staffUser);
            return userData.emp_code || userData.employee_code || userData.id || 'staff';
        } catch (e) {
            console.warn('Could not parse staff_user from localStorage');
        }
    }
    
    // Fallback for MNR users with userId:token format
    if (window.sessionToken && window.sessionToken.includes(':')) {
        const userId = window.sessionToken.split(':')[0];
        return userId;
    }
    
    // Final fallback - return 'admin' for authenticated staff
...
async function fetchDirectAwards() {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/direct-tiers?${queryParams}`);
...
async function fetchMatchingAwards() {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/matching-tiers?${queryParams}`);
        const result = await response.json();
...
async function fetchBonanzas() {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/bonanzas?${queryParams}`);
        const result = await response.json();
...
async function fetchAwardsDropdown() {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/awards-dropdown?user_id=${userId}`);
        const result = await response.json();
...
async function fetchSummary() {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/summary?user_id=${userId}`);
        const result = await response.json();
...
async function updateDirectAward(tierId, updateData) {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/direct-tiers/${tierId}?user_id=${userId}`, {
            method: 'PUT',
            headers: {
...
async function updateMatchingAward(tierId, updateData) {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/matching-tiers/${tierId}?user_id=${userId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function createBonanza(bonanzaData) {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/bonanzas?user_id=${userId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function updateBonanza(bonanzaId, updateData) {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/bonanzas/${bonanzaId}?user_id=${userId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function deleteBonanza(bonanzaId, reason) {
    try {
        ...
        const response = await staffFetch(`/api/v1/rvz/awards-config/bonanzas/${bonanzaId}?user_id=${userId}&deletion_reason=${encodeURIComponent(reason)}`, {
            method: 'DELETE'
        });
