...
def get_active_nda_for_staff_type(db, staff_type: str, document_type: str = 'NDA'):
    ...
    # Fetch active versions for document_type in a single query
    versions = db.query(StaffNdaVersion).filter(
        StaffNdaVersion.status == 'active',
    ...
    for v in versions:
        if v.applicable_staff_types and staff_type in v.applicable_staff_types:
            return v
            
    # Priority 2: global version of this document_type (empty/NULL = applies to all)
    for v in versions:
        if not v.applicable_staff_types:
            return v
...
DEFAULT_STAFF_MENUS = [
    ...
    {"menu_code": "STAFF_CATALOG_LIBRARY", "menu_name": "Catalog Library", "menu_category": "crm", "menu_icon": "fas fa-book-open", "route_path": "/staff/catalog-library", "display_order": 288, "is_default_visible": True, "is_default_accessible": True, "audience_scope": "staff", "sidebar_section": "crm", "sidebar_section_title": "CRM & LEADS", "sidebar_section_order": 4},
