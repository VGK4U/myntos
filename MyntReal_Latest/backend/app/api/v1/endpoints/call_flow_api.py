...
def _is_placeholder_name(name: Optional[str]) -> bool:
    ...
    if not n or n in ('guest caller', 'guest customer', 'customer', 'customer lead', 'lead', 'unknown', 'none', 'null', '-', 'unassigned'):
        return True
    if n.startswith(('missed call', 'incoming call', 'call from', 'call ', 'lead ', 'test lead')):
        return True
    digits_only = re.sub(r'\D', '', n)
    if digits_only and len(digits_only) >= 8 and len(re.sub(r'[\d\+\s\-\(\)]', '', n)) == 0:
        return True
    return False
...
def _resolve_contacts_batch(db: Session, phone_list: List[str], company_id: Optional[int] = None) -> Dict[str, Dict[str, Any]]:
    ...
    1. CRM Leads & CRM Lead Phone Records (Highest Priority)
    2. Synced Mobile Contacts from StaffCallLog (55,000+ native device contacts)
    3. OfficialPartner (VGK Members & Business Partners)
    4. Registered Members (User model)
    5. WhatsApp Profile Sender Names (wa_inbox)
    6. Partner Walk-ins & Internal Staff
    Promotes genuine human contact names over autogenerated placeholders (e.g. 'Missed Call 98765...').
    """
    if not phone_list:
        return {}
    ...
    # Helper for chunking list
    def _chunks(lst, n=100):
        for i in range(0, len(lst), n):
            yield lst[i:i + n]

    # 1. CRM Leads (Direct phone and alternate_phone)
    for chunk in _chunks(clean_digits):
        lead_filters = []
        for num in chunk:
        ...
        if lead_filters:
            ...
            for l in leads:
                ...
                for p_dig in p_nums:
                    if not p_dig:
                        continue
                    l_name = (l.name or "").strip()
                    is_real = bool(l_name and not _is_placeholder_name(l_name))
                    current_entry = resolved.get(p_dig)
                    has_real_current = bool(current_entry and not _is_placeholder_name(current_entry.get("name")))

                    if (p_dig not in resolved) or (not has_real_current and is_real):
                        resolved[p_dig] = {
                            "id": l.id,
                            "name": l_name if l_name else "Lead",
                            "source": "CRM Lead",
                            "email": l.email,
    ...
    # 1b. CRM Lead Phones (Normalized multi-phone table)
    unresolved_crm = [d for d in clean_digits if d not in resolved or _is_placeholder_name(resolved[d].get("name"))]
    if unresolved_crm:
        for chunk in _chunks(unresolved_crm):
            clp_filters = [CRMLeadPhone.phone_norm.ilike(f"%{num}%") for num in chunk]
            if clp_filters:
                clp_rows = db.query(CRMLeadPhone.phone_norm, CRMLead).join(
                    CRMLead, CRMLead.id == CRMLeadPhone.lead_id
                ).filter(or_(*clp_filters)).all()
                for p_num, l in clp_rows:
                    p_dig = re.sub(r'\D', '', p_num or '')[-10:]
                    if not p_dig:
                        continue
                    l_name = (l.name if l else "") or ""
                    l_name = l_name.strip()
                    is_real = bool(l_name and not _is_placeholder_name(l_name))
                    current_entry = resolved.get(p_dig)
                    has_real_current = bool(current_entry and not _is_placeholder_name(current_entry.get("name")))
                    ...
                    if (p_dig not in resolved) or (not has_real_current and is_real):
                        resolved[p_dig] = {
                            ...
                            "name": l_name if l_name else "Lead",
                            "source": "CRM Lead",
                            "email": l.email if l else None,
                            "status": str(l.status) if (l and hasattr(l, 'status') and l.status) else (getattr(l, 'lead_status', None) if l else "Lead"),
                            "city": getattr(l, 'city', None) if l else None,
                            "vertical": (getattr(l, 'vertical', None) or getattr(l, 'category', None) or "CRM Lead") if l else "CRM Lead",
                            "handler_id": getattr(l, 'handler_id', None) if l else None,
                            "telecaller_id": getattr(l, 'telecaller_id', None) if l else None,
                            "primary_owner_id": getattr(l, 'primary_owner_id', None) if l else None,
                            "assigned_to": getattr(l, 'assigned_to', None) if l else None
                        }

    # 2. Synced Mobile Contacts (StaffCallLog)
    # Also resolve for items where current name is an autogenerated placeholder (e.g. 'Missed Call ...')
    unresolved = [d for d in clean_digits if d not in resolved or _is_placeholder_name(resolved[d].get("name"))]
    if unresolved:
        for chunk in _chunks(unresolved):
            scl_filters = [StaffCallLog.phone_number.ilike(f"%{num}%") for num in chunk]
            if scl_filters:
                ...
                for row in scls:
                    ...
                    c_name = (row.contact_name or "").strip()
                    if p_dig and not _is_placeholder_name(c_name):
                        if p_dig in resolved:
                            if _is_placeholder_name(resolved[p_dig].get("name")):
                                resolved[p_dig]["name"] = c_name
                        else:
                            resolved[p_dig] = {
                                "id": None,
                                "name": c_name,
    ...
    if unresolved:
        ...
        for chunk in _chunks(unresolved):
            p_filters = [OfficialPartner.phone.ilike(f"%{num}%") for num in chunk]
            if p_filters:
                partners = db.query(
                    OfficialPartner.id,
                ...
                for p in partners:
                    ...
                    if p_dig and not _is_placeholder_name(p_name):
                        is_vgk = (p.category == 'VGK_TEAM')
                        if p_dig in resolved:
                            if _is_placeholder_name(resolved[p_dig].get("name")):
                                resolved[p_dig]["name"] = p_name
                        else:
                            resolved[p_dig] = {
                                "id": p.id,
                                "name": p_name,
    ...
    if unresolved:
        for chunk in _chunks(unresolved):
            u_filters = [User.phone_number.ilike(f"%{num}%") for num in chunk]
            if u_filters:
                try:
                    users = db.query(User).filter(or_(*u_filters)).all()
                    for u in users:
                        p_dig = re.sub(r'\D', '', u.phone_number or '')[-10:]
                        u_name = (u.name or "").strip()
                        if p_dig and not _is_placeholder_name(u_name):
                            if p_dig in resolved:
                                if _is_placeholder_name(resolved[p_dig].get("name")):
                                    resolved[p_dig]["name"] = u_name
                            else:
                                resolved[p_dig] = {
                                    "id": u.id,
                                    "name": u_name,
    ...
    # 5. WhatsApp Inbox Sender Profile Names (wa_inbox)
    unresolved = [d for d in clean_digits if d not in resolved or _is_placeholder_name(resolved[d].get("name"))]
    if unresolved:
        for chunk in _chunks(unresolved):
            wa_filters = [WAInbox.from_phone.ilike(f"%{num}%") for num in chunk]
            if wa_filters:
                try:
                    wa_rows = db.query(WAInbox.from_phone, WAInbox.from_name).filter(
                        or_(*wa_filters),
                        WAInbox.from_name.isnot(None),
                        WAInbox.from_name != '',
                        ~WAInbox.from_name.ilike('%system%'),
                        ~WAInbox.from_name.ilike('%unknown%')
                    ).order_by(WAInbox.id.desc()).all()
                    for row in wa_rows:
                        p_dig = re.sub(r'\D', '', row[0] or '')[-10:]
                        w_name = (row[1] or '').strip()
                        if p_dig and not _is_placeholder_name(w_name):
                            if p_dig in resolved:
                                if _is_placeholder_name(resolved[p_dig].get("name")):
                                    resolved[p_dig]["name"] = w_name
                            else:
                                resolved[p_dig] = {
                                    "id": None,
                                    "name": w_name,
    ...
    if unresolved:
        for chunk in _chunks(unresolved):
            staff_filters = [StaffEmployee.phone.ilike(f"%{num}%") for num in chunk]
            if staff_filters:
                ...
                for st in staffs:
                    ...
                    if p_dig and not _is_placeholder_name(st_name):
                        if p_dig in resolved:
                            if _is_placeholder_name(resolved[p_dig].get("name")):
                                resolved[p_dig]["name"] = st_name
                        else:
                            resolved[p_dig] = {
                                "id": st.id,
                                "name": st_name,
        ...
        # Check partner_walkins table via raw text query
        unresolved_wi = [d for d in clean_digits if d not in resolved or _is_placeholder_name(resolved[d].get("name"))]
        if unresolved_wi:
            for d in unresolved_wi[:50]:
                try:
                    wi_row = db.execute(
                        text("SELECT customer_name FROM partner_walkins WHERE (customer_phone LIKE :p OR alternate_phone LIKE :p) AND customer_name IS NOT NULL AND customer_name != '' LIMIT 1"),
                        {"p": f"%{d}%"}
                    ).fetchone()
                    if wi_row and wi_row[0] and not _is_placeholder_name(wi_row[0]):
                        wi_name = str(wi_row[0]).strip()
                        if d in resolved:
                            if _is_placeholder_name(resolved[d].get("name")):
                                resolved[d]["name"] = wi_name
                        else:
                            resolved[d] = {
                                "id": None,
                                "name": wi_name,
...
def list_incoming_calls(
    ...
    # Pre-resolve candidate phones and lead IDs when searching by customer, caller, lead, or staff name
    matching_phones = set()
    matching_lead_ids = set()
    matching_staff_ids = set()
    s_clean = search.strip().replace('+', '') if search else None

    if s_clean:
        if s_clean.isdigit():
            try:
                matching_lead_ids.add(int(s_clean))
            except Exception:
                pass
            clean_d = s_clean[-10:]
            if len(clean_d) >= 6:
                matching_phones.add(clean_d)

        # 1. CRM Leads by name, phone, alternate_phone
        crm_matches = db.query(CRMLead.id, CRMLead.phone, CRMLead.alternate_phone).filter(
            or_(
                CRMLead.name.ilike(f"%{s_clean}%"),
                CRMLead.phone.ilike(f"%{s_clean}%"),
                CRMLead.alternate_phone.ilike(f"%{s_clean}%")
            )
        ).limit(200).all()
        for lid, p1, p2 in crm_matches:
            matching_lead_ids.add(lid)
            if p1:
                d1 = re.sub(r'\D', '', str(p1))[-10:]
                if len(d1) >= 6: matching_phones.add(d1)
            if p2:
                d2 = re.sub(r'\D', '', str(p2))[-10:]
                if len(d2) >= 6: matching_phones.add(d2)

        # 1b. CRMLeadPhone by phone_norm
        try:
            from app.models.crm import CRMLeadPhone
            clp_matches = db.query(CRMLeadPhone.lead_id, CRMLeadPhone.phone_norm).filter(
                CRMLeadPhone.phone_norm.ilike(f"%{s_clean}%")
            ).limit(100).all()
            for lid, p_num in clp_matches:
                if lid: matching_lead_ids.add(lid)
                if p_num:
                    d = re.sub(r'\D', '', str(p_num))[-10:]
                    if len(d) >= 6: matching_phones.add(d)
        except Exception:
            pass

        # 2. StaffCallLog by contact_name or phone
        try:
            from app.models.call_tracking import StaffCallLog
            scl_matches = db.query(StaffCallLog.phone_number, StaffCallLog.matched_lead_id).filter(
                or_(
                    StaffCallLog.contact_name.ilike(f"%{s_clean}%"),
                    StaffCallLog.phone_number.ilike(f"%{s_clean}%")
                )
            ).limit(100).all()
            for p_num, mlid in scl_matches:
                if mlid: matching_lead_ids.add(mlid)
                if p_num:
                    d = re.sub(r'\D', '', str(p_num))[-10:]
                    if len(d) >= 6: matching_phones.add(d)
        except Exception:
        ...
        # 3. User by name or phone
        try:
            from app.models.user import User
            u_matches = db.query(User.phone_number).filter(
                or_(
                    User.name.ilike(f"%{s_clean}%"),
                    User.phone_number.ilike(f"%{s_clean}%")
                )
            ).limit(100).all()
            for (up,) in u_matches:
                if up:
                    d = re.sub(r'\D', '', str(up))[-10:]
                    if len(d) >= 6: matching_phones.add(d)
        except Exception:
            pass

        # 4. OfficialPartner by partner_name or phone
        try:
            from app.models.staff_accounts import OfficialPartner
            op_matches = db.query(OfficialPartner.phone).filter(
                or_(
                    OfficialPartner.partner_name.ilike(f"%{s_clean}%"),
                    OfficialPartner.phone.ilike(f"%{s_clean}%")
                )
            ).limit(100).all()
            for (op_p,) in op_matches:
                if op_p:
                    d = re.sub(r'\D', '', str(op_p))[-10:]
                    if len(d) >= 6: matching_phones.add(d)
        except Exception:
            pass

        # 5. StaffEmployee by full_name, first_name, last_name, emp_code
        try:
            st_matches = db.query(StaffEmployee.id).filter(
                or_(
                    StaffEmployee.full_name.ilike(f"%{s_clean}%"),
                    StaffEmployee.first_name.ilike(f"%{s_clean}%"),
                    StaffEmployee.last_name.ilike(f"%{s_clean}%"),
                    StaffEmployee.emp_code.ilike(f"%{s_clean}%")
                )
            ).limit(50).all()
            for (st_id,) in st_matches:
                matching_staff_ids.add(st_id)
        except Exception:
            pass
    ...
    if scope_clean in ("new_calls", "new"):
        ...
        if s_clean:
            v_clauses = [
                ...
                VoIPCallSession.operator_name.ilike(f"%{s_clean}%"),
                VoIPCallSession.metadata_json.ilike(f"%{s_clean}%")
            ]
            if matching_staff_ids:
                v_clauses.append(VoIPCallSession.operator_id.in_(list(matching_staff_ids)))
            if matching_lead_ids:
                v_clauses.append(VoIPCallSession.lead_id.in_(list(matching_lead_ids)))
            if matching_phones:
                for mp in list(matching_phones)[:50]:
                    v_clauses.append(VoIPCallSession.customer_phone.ilike(f"%{mp}%"))
                    v_clauses.append(VoIPCallSession.destination_number.ilike(f"%{mp}%"))
            query = query.filter(or_(*v_clauses))
    ...
    # ── BRANCH B: Unified Multi-Source Call History ('my', 'team', 'overall') ───
        ...
        if s_clean:
            ...
            if matching_phones:
                for mp in list(matching_phones)[:50]:
                    v_clauses.append(VoIPCallSession.customer_phone.ilike(f"%{mp}%"))
                    v_clauses.append(VoIPCallSession.destination_number.ilike(f"%{mp}%"))
            voip_q = voip_q.filter(or_(*v_clauses))
        ...
        if not did_number and (not call_type or call_type.lower() != 'voicemail'):
            ...
            if s_clean:
                ...
                if matching_staff_ids:
                    scl_clauses.append(StaffCallLog.staff_id.in_(list(matching_staff_ids)))
                if matching_lead_ids:
                    scl_clauses.append(StaffCallLog.matched_lead_id.in_(list(matching_lead_ids)))
                if matching_phones:
                    for mp in list(matching_phones)[:50]:
                        scl_clauses.append(StaffCallLog.phone_number.ilike(f"%{mp}%"))
                scl_q = scl_q.filter(or_(*scl_clauses))
    ...
    for c in voip_items:
        ...
        if c.metadata_json and ('original_caller_number' in c.metadata_json or 'caller_identity' in c.metadata_json):
            try:
                mj = json.loads(c.metadata_json)
                ci = mj.get('caller_identity') or {}
                orig = ci.get('original_caller_number')
                if orig:
                    orig_digits = re.sub(r'\D', '', str(orig))[-10:]
                    if orig_digits:
                        phone_clean_list.append(orig_digits)
            except Exception:
                pass
    for s in scl_items:
        if s.matched_lead_id:
            all_lead_ids.add(s.matched_lead_id)
        digits = re.sub(r'\D', '', s.phone_number or '')[-10:]
        if digits:
            phone_clean_list.append(digits)
    for r in att_rows:
        if r[2]:
            all_lead_ids.add(r[2])
    ...
    # Pre-fetch CRM Leads by ID to ensure accurate lead name and owner resolution
    lead_id_map = {}
    if all_lead_ids:
        try:
            leads_by_id = db.query(CRMLead).filter(CRMLead.id.in_(all_lead_ids)).all()
            for l in leads_by_id:
                if l.name and not _is_placeholder_name(l.name):
                    lead_id_map[l.id] = {
                        "name": l.name,
                        "phone": l.phone,
                        "primary_owner_id": l.primary_owner_id,
                        "telecaller_id": l.telecaller_id,
                        "handler_id": l.handler_id,
                        "assigned_to": getattr(l, 'assigned_to', None),
                        "vertical": getattr(l, 'vertical', 'CRM Lead'),
                        "source": "CRM Lead",
                        "id": l.id
                    }
        except Exception as l_err:
            logger.warning(f"[CALL-FLOW-UNIFIED] lead_id_map error: {l_err}")
    ...
    for c in voip_items:
        ...
        if c.lead_id and (not contact_match or _is_placeholder_name(contact_match.get('name'))):
            lead_info = lead_id_map.get(c.lead_id)
            if lead_info:
                contact_match = lead_info
        ...
        if original_caller_number and (not contact_match or _is_placeholder_name(contact_match.get('name'))):
            orig_10 = re.sub(r'\D', '', str(original_caller_number))[-10:]
            if orig_10 and orig_10 in contact_dict and not _is_placeholder_name(contact_dict[orig_10].get('name')):
                contact_match = contact_dict[orig_10]
        ...
        resolved_name = None
        if contact_match and not _is_placeholder_name(contact_match.get('name')):
            resolved_name = contact_match.get('name')
        elif c.lead_id and lead_id_map.get(c.lead_id):
            resolved_name = lead_id_map[c.lead_id].get('name')
        if not resolved_name:
            cand_meta = meta_dict.get("customer_name") or meta_dict.get("contact_name")
            if cand_meta and not _is_placeholder_name(cand_meta):
                resolved_name = cand_meta
        if not resolved_name:
            resolved_name = "Guest Caller"

        effective_lead_id = c.lead_id or (contact_match['id'] if (contact_match and contact_match.get('source') == 'CRM Lead') else None)
        ...
        res_items.append({
            ...
            "crm_lead_id": effective_lead_id,
            "contact_source": contact_match['source'] if contact_match else ('CRM Lead' if c.lead_id else None),
    ...
    for s in scl_items:
        ...
        clean_10 = re.sub(r'\D', '', raw_customer_num)[-10:] if raw_customer_num else ""
        contact_match = contact_dict.get(clean_10)
        if s.matched_lead_id and (not contact_match or _is_placeholder_name(contact_match.get('name'))):
            lead_info = lead_id_map.get(s.matched_lead_id)
            if lead_info:
                contact_match = lead_info
        ...
        effective_scl_name = None
        if scl_name_raw and not _is_placeholder_name(scl_name_raw) and scl_name_raw.lower() not in ('unknown', 'none', 'null', '-'):
            effective_scl_name = scl_name_raw
        elif contact_match and not _is_placeholder_name(contact_match.get('name')):
            effective_scl_name = contact_match.get('name')
        elif s.matched_lead_id and lead_id_map.get(s.matched_lead_id):
            effective_scl_name = lead_id_map[s.matched_lead_id].get('name')
        else:
            effective_scl_name = "Lead"
        ...
        res_items.append({
            ...
            "customer_phone_masked": _mask_phone(raw_customer_num, current_user),
    ...
    for r in att_rows:
        ...
        clean_10 = re.sub(r'\D', '', raw_customer_num)[-10:] if raw_customer_num else ""
        contact_match = contact_dict.get(clean_10)
        if lid and (not contact_match or _is_placeholder_name(contact_match.get('name'))):
            lead_info = lead_id_map.get(lid)
            if lead_info:
                contact_match = lead_info
        ...
        resolved_att_name = None
        if contact_match and not _is_placeholder_name(contact_match.get('name')):
            resolved_att_name = contact_match.get('name')
        elif lname and not _is_placeholder_name(lname):
            resolved_att_name = lname
        elif lid and lead_id_map.get(lid):
            resolved_att_name = lead_id_map[lid].get('name')
        else:
            resolved_att_name = "Lead"
        ...
        res_items.append({
            ...
            "customer_phone_display": _format_phone(raw_customer_num),
            "customer_phone_masked": _mask_phone(raw_customer_num, current_user),
            "customer_name": resolved_att_name,
            "crm_lead_id": lid or (contact_match['id'] if (contact_match and contact_match.get('source') == 'CRM Lead') else None),
...
def get_customer_call_history(
    ...
    return {
        ...
        "customer_phone_display": _format_phone(clean_digits),
        "customer_name": resolved_name,
        "lead": {
            "id": contact_info.get("id") if contact_info else None,
            "name": resolved_name,
            "phone": clean_digits,
            "company_id": contact_info.get("company_id") if contact_info else company_id,
