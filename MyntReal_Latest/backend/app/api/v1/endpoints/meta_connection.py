...
def get_meta_connection_status(
    ...
    return get_meta_connection_dashboard_status(db, target_cid)
...
def read_meta_real_assets(
    ...
    """
    target_cid = resolve_tenant_company_for_request(current_user, company_id)
    return read_real_meta_account_assets(db, target_cid)
...
def connect_meta_token(
    ...
    """
    Connect a new Meta Page or User Access Token with encrypted storage at rest.
    """
...
def get_first_campaign_readiness_status(
    ...
    return evaluate_first_campaign_readiness(db, target_cid)
...
def create_first_controlled_meta_campaign(
    ...
    return execute_first_live_meta_campaign_creation(db, target_cid)
...
def get_asset_mapping_preflight_report(
    ...
    return evaluate_phase2g_preflight_checks(db, target_cid)
